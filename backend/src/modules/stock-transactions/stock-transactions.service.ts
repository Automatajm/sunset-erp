...
export class StockTransactionsService {
  ...
  async findAll(
    tenantId: string,
    filters?: { itemId?: string; warehouseId?: string; transactionType?: string },
  ) {
    ...
    const rows = movements.map((m) => ({
      ...
      referenceDisplay:
        m.referenceType === 'CYCLE_COUNT' && m.referenceId
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        await tx.stockMovement.create({
          data: {
            ...
            movementValue: new Decimal(
              Math.round(allQtys.purchaseQty * line.unitPrice * 100) / 100,
            ),
  ...
  async shipFromArInvoice(
    ...
    for (const line of arInvoice.lines) {
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        const cogsValue = this.uom.calcFinancialValue(allQtys.consumptionQty, 'consumption', {
          unitCost,
  ...
  async getLedger(
    ...
    let filtered = movements.filter(
      (m) => !filters?.itemType || m.item?.itemType === filters.itemType,
    );
    ...
    const arIds = [
      ...new Set(
        filtered
          .filter((m) => m.referenceType === 'ar_invoice' && m.referenceId)
          .map((m) => m.referenceId!),
      ),
    ];
    const apIds = [
      ...new Set(
        filtered
          .filter((m) => m.referenceType === 'ap_invoice' && m.referenceId)
          .map((m) => m.referenceId!),
      ),
    ];
    const poIds = [
    ...
    const grnIds = [
      ...new Set(
        filtered
          .filter(
            (m) => (m.referenceType === 'GRN' || m.referenceType === 'GRN_CANCEL') && m.referenceId,
          )
          .map((m) => m.referenceId!),
      ),
    ];
    ...
    const arMap = new Map<string, string>(
      arInvoices.map((i) => [i.id, i.invoiceNumber] as [string, string]),
    );
    const apMap = new Map<string, string>(
      apInvoices.map((i) => [i.id, i.invoiceNumber] as [string, string]),
    );
    const poMap = new Map<string, string>(
      (poOrders as any[]).map((p) => [p.id, p.poNumber] as [string, string]),
    );
    const grnMap = new Map<string, string>(
      (grnReceipts as any[]).map((g) => [g.id, g.grnNumber] as [string, string]),
    );
    const cycleMap = new Map<string, string>(
      (cycleSessions as any[]).map((s) => [s.id, s.sessionNumber] as [string, string]),
    );
    ...
    const resolveRef = (m: any): string => {
      ...
      if (m.referenceType === 'GRN_CANCEL')
        return (grnMap.get(m.referenceId) ?? m.referenceId) + ' (cancel)';
    ...
    const rows = filtered.map((m) => {
      ...
      const displayValue = storedValue !== null ? storedValue : isOut ? -totalValue : totalValue;
    ...
    const totalInValue = rows
      .filter((r) => r.signedQuantity > 0)
      .reduce((s, r) => s + r.totalValue, 0);
    const totalOutValue = rows
      .filter((r) => r.signedQuantity < 0)
      .reduce((s, r) => s + Math.abs(r.totalValue), 0);
  ...
  async getStockPlanning(
    tenantId: string,
    filters?: { warehouseId?: string; itemType?: string; alertOnly?: boolean },
  ) {
    ...
    const filtered = stockPositions.filter(
    ...
    const pendingPOLines = await this.prisma.purchaseOrderLine.findMany({
      ...
      include: {
        purchaseOrder: { select: { id: true, poNumber: true, status: true, expectedDate: true } },
      },
    });
    ...
    const openSOLines = await this.prisma.salesOrderLine.findMany({
      ...
      include: {
        salesOrder: { select: { id: true, soNumber: true, status: true, promisedDate: true } },
      },
    });

    const poSupplyMap = new Map<
      string,
      {
        totalPending: number;
    ...
    for (const line of pendingPOLines) {
      ...
      ex.orders.push({
        ...
        pending,
        expectedDate: line.purchaseOrder.expectedDate?.toISOString().split('T')[0] ?? null,
      });
      poSupplyMap.set(line.itemId, ex);
    }

    const soDemandMap = new Map<
      string,
      {
        totalDemand: number;
    ...
    for (const line of openSOLines) {
      ...
      ex.orders.push({
        ...
        demand,
        promisedDate: line.salesOrder.promisedDate?.toISOString().split('T')[0] ?? null,
      });
    ...
    const rows = filtered.map((s) => {
      ...
      const coverageDays =
        dailyDemand > 0 ? Math.floor(available / dailyDemand) : available > 0 ? 999 : 0;
  ...
  async getAbcAnalysis(tenantId: string, filters?: { warehouseId?: string; itemType?: string }) {
    ...
    return {
      ...
      summary: {
        ...
        totalItems: rows.length,
        classA: {
          count: rows.filter((r) => r.abcClass === 'A').length,
          value:
            Math.round(
              rows.filter((r) => r.abcClass === 'A').reduce((s, r) => s + r.totalValue, 0) * 100,
            ) / 100,
        },
        classB: {
          count: rows.filter((r) => r.abcClass === 'B').length,
          value:
            Math.round(
              rows.filter((r) => r.abcClass === 'B').reduce((s, r) => s + r.totalValue, 0) * 100,
            ) / 100,
        },
        classC: {
          count: rows.filter((r) => r.abcClass === 'C').length,
          value:
            Math.round(
              rows.filter((r) => r.abcClass === 'C').reduce((s, r) => s + r.totalValue, 0) * 100,
            ) / 100,
        },
      },
      asOf: new Date(),
  ...
  async getStockAging(tenantId: string, filters?: { warehouseId?: string; itemType?: string }) {
    ...
    const bucketSummary: Record<string, { count: number; value: number }> = {
    ...
    for (const r of rows) {
      ...
      bucketSummary[r.agingBucket].value =
        Math.round((bucketSummary[r.agingBucket].value + r.totalValue) * 100) / 100;
    }
    ...
    return {
      ...
      summary: {
        ...
        slowMovingValue:
          Math.round(
            rows.filter((r) => r.isSlowMoving).reduce((s, r) => s + r.totalValue, 0) * 100,
          ) / 100,
        deadStockCount: rows.filter((r) => r.isDead).length,
        deadStockValue:
          Math.round(rows.filter((r) => r.isDead).reduce((s, r) => s + r.totalValue, 0) * 100) /
          100,
  ...
  async getInventoryTurnover(
    tenantId: string,
    filters?: { warehouseId?: string; itemType?: string; dateFrom?: string; dateTo?: string },
  ) {
    const now = new Date();
