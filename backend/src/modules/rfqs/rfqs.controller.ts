...
export class RfqsController {
  ...
  @ApiResponse({ status: 200, description: 'Envelope { rfqs, count }' })
  async findAll(@Request() req, @Query() query: QueryRfqsDto) {
    return this.rfqsService.findAll(req.user.tenantId, query.status);
  }
  ...
  @ApiParam({ name: 'id', description: 'RFQ UUID' })
  @ApiResponse({ status: 200, description: 'Comparison matrix per line and supplier' })
  @ApiResponse({ status: 404, description: 'RFQ not found' })
  ...
  @ApiResponse({ status: 400, description: 'RFQ is not editable in its current status' })
  @ApiResponse({ status: 404, description: 'RFQ not found' })
  ...
  @ApiResponse({ status: 400, description: 'RFQ already awarded or cancelled' })
  @ApiResponse({ status: 404, description: 'RFQ not found' })
  ...
  @ApiParam({ name: 'id', description: 'RFQ UUID' })
  @ApiResponse({ status: 200, description: 'RFQ soft-deleted' })
  @ApiResponse({ status: 400, description: 'Only draft RFQs can be deleted' })
  @ApiResponse({ status: 404, description: 'RFQ not found' })
