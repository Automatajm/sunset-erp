...
export class AutomationService {
  ...
  async updateConfig(
    ...
    if (!AUTOMATION_MODULES.includes(module as AutomationModule)) {
      throw new BadRequestException(
        `Unknown module: ${module}. Valid: ${AUTOMATION_MODULES.join(', ')}`,
      );
    }
  ...
  ): Promise<'full_auto' | 'review_required' | 'manual'> {
  ...
  async approveQueueItem(
    ...
    return {
      message: `JE ${item.journalEntry.entryNumber} approved and posted`,
      queueId,
      jeId: item.jeId,
    };
  }
