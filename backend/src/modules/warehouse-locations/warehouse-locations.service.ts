// ============================================================================
// FILE 4 — backend/src/modules/warehouse-locations/warehouse-locations.service.ts
// ============================================================================
...
// ── DTOs inline (no dto/ subfolder needed — simple enough) ──────────────────
 
export class CreateZoneDto {
  warehouseId: string;
  code: string;         // STOR, RECV, QUAR, SHIP
  name: string;
  zoneType?: string;    // storage | receiving | shipping | quarantine | production | returns
  description?: string;
  isActive?: boolean;
}
...
export class CreateBinDto {
  ...
  code: string;
  name?: string;
  binType?: string;     // standard | pallet | big_bag | tank | silo | ibc | container | bulk
  maxWeightKg?: number;
  maxVolumeLtr?: number;
  maxPallets?: number;
...
export class WarehouseLocationsService {
  ...
  async updateZone(tenantId: string, userId: string, id: string, dto: Partial<CreateZoneDto>) {
    const zone = await this.prisma.warehouseZone.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!zone) throw new NotFoundException('Zone not found');
    return this.prisma.warehouseZone.update({ where: { id }, data: { ...dto, updatedBy: userId } });
  }
 
  async removeZone(tenantId: string, userId: string, id: string) {
    const zone = await this.prisma.warehouseZone.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!zone) throw new NotFoundException('Zone not found');
    await this.prisma.warehouseZone.update({ where: { id }, data: { deletedAt: new Date(), deletedBy: userId } });
    return { message: 'Zone deleted', id };
  }
 
  // ── AISLES — fullCode = ZONE.code + "-" + aisle.code → STOR-01 ─────────────
  ...
  async updateAisle(tenantId: string, userId: string, id: string, dto: Partial<CreateAisleDto>) {
    const aisle = await this.prisma.warehouseAisle.findFirst({
      where: { id, tenantId, deletedAt: null }, include: { zone: true },
    });
  ...
  async removeAisle(tenantId: string, userId: string, id: string) {
    const aisle = await this.prisma.warehouseAisle.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!aisle) throw new NotFoundException('Aisle not found');
    await this.prisma.warehouseAisle.update({ where: { id }, data: { deletedAt: new Date(), deletedBy: userId } });
    return { message: 'Aisle deleted', id };
  }
 
  // ── RACKS — fullCode = AISLE.fullCode + "-" + rack.code → STOR-01-02 ────────
  ...
  async updateRack(tenantId: string, userId: string, id: string, dto: Partial<CreateRackDto>) {
    const rack = await this.prisma.warehouseRack.findFirst({
      where: { id, tenantId, deletedAt: null }, include: { aisle: true },
    });
  ...
  async removeRack(tenantId: string, userId: string, id: string) {
    const rack = await this.prisma.warehouseRack.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!rack) throw new NotFoundException('Rack not found');
    await this.prisma.warehouseRack.update({ where: { id }, data: { deletedAt: new Date(), deletedBy: userId } });
    return { message: 'Rack deleted', id };
  }
 
  // ── LEVELS — fullCode = RACK.fullCode + "-" + level.code → STOR-01-02-01 ────
  ...
  async updateLevel(tenantId: string, userId: string, id: string, dto: Partial<CreateLevelDto>) {
    const level = await this.prisma.warehouseLevel.findFirst({
      where: { id, tenantId, deletedAt: null }, include: { rack: true },
    });
  ...
  async removeLevel(tenantId: string, userId: string, id: string) {
    const level = await this.prisma.warehouseLevel.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!level) throw new NotFoundException('Level not found');
    const hasBins = await this.prisma.warehouseBin.count({ where: { levelId: id, deletedAt: null } });
    if (hasBins > 0) throw new BadRequestException('Cannot delete level with active bins. Delete bins first.');
    await this.prisma.warehouseLevel.update({ where: { id }, data: { deletedAt: new Date(), deletedBy: userId } });
    return { message: 'Level deleted', id };
  }
 
  // ── BINS — fullCode = LEVEL.fullCode + "-" + bin.code → STOR-01-02-01-03 ────
  ...
  async updateBin(tenantId: string, userId: string, id: string, dto: Partial<CreateBinDto>) {
    const bin = await this.prisma.warehouseBin.findFirst({
      where: { id, tenantId, deletedAt: null }, include: { level: true },
    });
  ...
  async removeBin(tenantId: string, userId: string, id: string) {
    const bin = await this.prisma.warehouseBin.findFirst({ where: { id, tenantId, deletedAt: null } });
    if (!bin) throw new NotFoundException('Bin not found');
    const hasStock = await this.prisma.stock.count({ where: { binId: id, onHandQuantity: { gt: 0 } } });
    if (hasStock > 0) throw new BadRequestException('Cannot delete bin with stock on hand.');
    await this.prisma.warehouseBin.update({ where: { id }, data: { deletedAt: new Date(), deletedBy: userId } });
    return { message: 'Bin deleted', id };
  }
}