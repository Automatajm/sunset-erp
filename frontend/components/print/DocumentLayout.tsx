      ...
      >
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #1a1a1a', paddingBottom: 14, marginBottom: 18 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#c2410c', letterSpacing: '0.02em' }}>{number}</div>
            <div style={{ fontSize: 11, color: '#444', marginTop: 4 }}>Date: {fmtDate(date)}</div>
            {status && (
              <div style={{ marginTop: 6, display: 'inline-block', padding: '2px 10px', borderRadius: 4, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#c2410c', border: '1px solid #c2410c' }}>
                {status}
              </div>
            )}
          </div>
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      ...
      {totals && (
        <tfoot>
          ...
          {totals.total != null && (
            <tr><td colSpan={4} /><td style={{ ...rd, fontWeight: 700, color: '#111', borderTop: '1.5px solid #333' }}>Total</td><td style={{ ...rd, fontWeight: 700, color: '#c2410c', borderTop: '1.5px solid #333' }}>{money(totals.total, currency)}</td></tr>
          )}
        </tfoot>
      )}
    </table>
