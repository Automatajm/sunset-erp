...
const UOM_COLOR: Record<string, string> = {
  volume: 'var(--accent-blue, #60a5fa)', mass: 'var(--accent-violet, #a78bfa)', count: 'var(--success, #4ade80)', length: 'var(--warning, #fbbf24)', area: 'var(--accent-strong, #fb923c)',
};
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 14 }}>
              ...
              <tbody>
                {routingSteps.map(step => (
                  <tr key={step.id}>
                    <td style={{ padding: '6px 10px', ...MONO, color: 'var(--accent-blue, #60a5fa)', textAlign: 'center', width: 40 }}>{step.stepNumber}</td>
                    ...
                    <td style={{ padding: '6px 10px' }}>
                      <button onClick={() => handleDeleteStep(step.id)} disabled={deletingStep === step.id}
                        style={{ padding: '3px 8px', borderRadius: 5, fontSize: 11, cursor: 'pointer', color: 'var(--danger, #f87171)', background: 'rgba(248,113,113,0.08)', border: '0.5px solid rgba(248,113,113,0.15)', fontFamily: "'IBM Plex Sans',sans-serif", opacity: deletingStep === step.id ? 0.5 : 1 }}>
...
function BOMModal({ items, consumptionGroups, systemUoms, onClose, onSaved }: {
  items: Item[];
    ...
    setComponents(cs => cs.map((c, i) => {
      ...
      if (k === 'consumptionGroupId') {
        ...
        if (cg) {
          ...
          // Default uom to the group's system UOM code
          if (cg.consumptionUom?.code) updated.uom = cg.consumptionUom.code;
        }
      }
      return updated;
    }));
          ...
          <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--w25, rgba(255,255,255,0.25))', padding: '4px 0 2px', borderBottom: '0.5px solid var(--l06, rgba(255,255,255,0.06))', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button type="button"
                ...
                style={{ background: 'var(--l04, rgba(255,255,255,0.04))', border: '0.5px solid var(--w10, rgba(255,255,255,0.1))', borderRadius: 5, padding: '3px 10px', fontSize: 11, color: 'var(--w50, rgba(255,255,255,0.5))', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>
                + Add row
              </button>
            </div>
            ...
            {components.map((c, idx) => {
              ...
              const cgConsUomId = selCg?.consumptionUomId ?? '';
              return (
                <div key={idx} style={{ display: 'grid', gridTemplateColumns: '2fr 80px 90px 1fr 70px 24px', gap: 6, alignItems: 'start' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    />
                    {selCg && (
                      <span style={{ fontSize: 10, color: 'var(--w30, rgba(255,255,255,0.3))' }}>{selCg.name}</span>
                    )}
                  </div>
                  {/* Qty Per */}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    {systemUoms.length === 0 ? (
                      <div style={{ fontSize: 11, color: 'var(--w20, rgba(255,255,255,0.2))', padding: '7px 8px', background: 'var(--l02, rgba(255,255,255,0.02))', borderRadius: 7, border: '0.5px solid var(--l06, rgba(255,255,255,0.06))' }}>
                        No system UOMs
                      </div>
                    ) : (
                      <SearchSelect
                  ...
                  {components.length > 1 && (
                    <button type="button"
                      ...
                      style={{ width: 22, height: 22, borderRadius: 4, background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.2)', color: 'var(--danger, #f87171)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 6 }}>
                      ×
                    </button>
                  )}
                </div>
  ...
  const columns = useMemo<ERPTreeColumn<Bom>[]>(() => [
    {
      ...
      value: r => r.bomNumber,
      render: r => <span style={{ ...MONO, color: 'var(--accent-strong, #fb923c)', fontWeight: 500 }}>{r.bomNumber}</span>,
    },
    {
    ...
    },
    {
      ...
      render: r => (
        <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, color: r.isActive ? 'var(--success, #4ade80)' : 'var(--danger, #f87171)', background: r.isActive ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)', border: `0.5px solid ${r.isActive ? 'rgba(74,222,128,0.2)' : 'rgba(248,113,113,0.2)'}` }}>
    ...
    },
    {
      ...
      render: r => (
        <button onClick={e => { e.stopPropagation(); bomApi.remove(r.id).then(fetchBoms); }}
          style={{ padding: '4px 9px', borderRadius: 6, fontSize: 11, cursor: 'pointer', background: 'rgba(239,68,68,0.08)', color: 'var(--danger, #f87171)', border: '0.5px solid rgba(239,68,68,0.2)', fontFamily: "'IBM Plex Sans',sans-serif" }}>
          Delete
        </button>
      ),
    },
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Manufacturing', 'Bill of Materials']} title="Bill of Materials">
      <style>{`
        ...
        .bom-btn-new{display:flex;align-items:center;gap:6px;background:linear-gradient(135deg,var(--accent-pressed, #c2410c),var(--accent, #ea580c),var(--accent-mid, #f97316));border:none;border-radius:7px;padding:7px 14px;font-size:12px;font-weight:500;font-family:'IBM Plex Sans',sans-serif;color:white;cursor:pointer;box-shadow:0 3px 12px rgba(234,88,12,0.3)}
        .bom-error{background:rgba(239,68,68,0.08);border:0.5px solid rgba(239,68,68,0.2);border-radius:8px;padding:10px 14px;margin-bottom:10px;font-size:13px;color:var(--danger-subtle, #fca5a5);flex-shrink:0}
      `}</style>
