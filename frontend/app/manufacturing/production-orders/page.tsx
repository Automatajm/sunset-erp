...
const PRIORITY_COLOR: Record<string, string> = { low: 'var(--w30, rgba(255,255,255,0.3))', medium: 'var(--warning, #fbbf24)', high: 'var(--accent-strong, #fb923c)', urgent: 'var(--danger, #f87171)' };
      ...
      <div style={{ background: 'var(--surface, #0e0b1a)', border: '0.5px solid rgba(74,222,128,0.2)', borderRadius: 14, width: '100%', maxWidth: 460, boxShadow: '0 24px 60px rgba(0,0,0,0.7)' }}>
        <div style={{ padding: '14px 18px 10px', borderBottom: '0.5px solid var(--l06, rgba(255,255,255,0.06))', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--w35, rgba(255,255,255,0.35))', marginTop: 2 }}>{mo.poNumber} · Planned: <span style={{ color: 'var(--accent-strong, #fb923c)', fontFamily: "'IBM Plex Mono',monospace" }}>{fmtNum(planned)}</span> units</div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {form.quantityDelivered && (
              <div style={{ background: 'var(--l03, rgba(255,255,255,0.03))', border: `0.5px solid ${variance === 0 ? 'var(--l08, rgba(255,255,255,0.08))' : variance < 0 ? 'rgba(248,113,113,0.2)' : 'rgba(74,222,128,0.2)'}`, borderRadius: 7, padding: '8px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 11, color: 'var(--w40, rgba(255,255,255,0.4))' }}>Variance</span>
                <span style={{ ...MONO, color: variance === 0 ? 'var(--w50, rgba(255,255,255,0.5))' : variance < 0 ? 'var(--danger, #f87171)' : 'var(--success, #4ade80)', fontWeight: 500 }}>
...
function ActualsPanel({ mo, onRefresh }: { mo: ProductionOrder; onRefresh: () => void }) {
  ...
  return (
    <div style={{ padding: '10px 40px 16px', background: 'var(--l01, rgba(255,255,255,0.01))', borderTop: '0.5px solid var(--l04, rgba(255,255,255,0.04))' }}>
      ...
      {loading ? (
        <div style={{ fontSize: 12, color: 'var(--w30, rgba(255,255,255,0.3))', padding: '8px 0' }}>Loading…</div>
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10, flexWrap: 'wrap', gap: 10 }}>
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {[
                { label: 'Planned',    value: `${laborData.summary.totalPlannedHours}h`,                                              color: 'var(--w50, rgba(255,255,255,0.5))' },
          ...
          {routingSuggestions && routingSuggestions.length > 0 && (
            <div style={{ background: 'rgba(167,139,250,0.06)', border: '0.5px solid rgba(167,139,250,0.2)', borderRadius: 8, padding: '10px 14px', marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button onClick={() => setRoutingSelected(new Set())}
                    style={{ fontSize: 11, color: 'var(--w30, rgba(255,255,255,0.3))', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>
                    Clear
                  </button>
                </div>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, marginBottom: 10 }}>
                <thead>
                  <tr>
                    <th style={{ width: 30, padding: '3px 8px', borderBottom: '0.5px solid var(--l04, rgba(255,255,255,0.04))' }}></th>
                    {['Step', 'Description', 'Work Center', 'Setup (h)', 'Run (h)', 'Total (h)', 'Est. Cost'].map(h => (
                      <th key={h} style={{ padding: '3px 8px', fontSize: 10, color: 'rgba(167,139,250,0.5)', fontWeight: 500, textTransform: 'uppercase', textAlign: 'left', borderBottom: '0.5px solid var(--l04, rgba(255,255,255,0.04))' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {routingSuggestions.map((s: any) => {
                    ...
                    return (
                      <tr key={s.stepNumber} style={{ opacity: checked ? 1 : 0.4, cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '5px 8px', fontSize: 12, color: 'var(--w55, rgba(255,255,255,0.55))' }}>{s.workCenter?.name}</td>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <button onClick={() => { setRoutingSuggestions(null); setRoutingSelected(new Set()); }}
                  style={{ padding: '5px 14px', borderRadius: 6, fontSize: 12, cursor: 'pointer', color: 'var(--w40, rgba(255,255,255,0.4))', background: 'var(--l04, rgba(255,255,255,0.04))', border: '0.5px solid var(--w10, rgba(255,255,255,0.1))', fontFamily: "'IBM Plex Sans',sans-serif" }}>
                  Dismiss
                </button>
                <span style={{ fontSize: 11, color: 'var(--w25, rgba(255,255,255,0.25))' }}>{routingSelected.size} of {routingSuggestions.length} selected</span>
              </div>
            </div>
          )}
          {routingSuggestions && routingSuggestions.length === 0 && (
          ...
          {laborData.actuals.length === 0 ? (
            <div style={{ fontSize: 12, color: 'var(--w25, rgba(255,255,255,0.25))' }}>No labor actuals recorded yet.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>{['Date', 'Employee', 'Planned', 'Actual', 'Variance', 'Rate', 'Cost', 'Notes'].map(h => (
                  <th key={h} style={{ padding: '4px 10px', fontSize: 10, color: 'rgba(167,139,250,0.5)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: 'left', borderBottom: '0.5px solid var(--l04, rgba(255,255,255,0.04))' }}>{h}</th>
                ))}</tr>
              </thead>
              <tbody>
                {laborData.actuals.map(a => {
                  ...
                  return (
                    <tr key={a.id}>
                      <td style={{ padding: '5px 10px', color: 'var(--w45, rgba(255,255,255,0.45))' }}>{fmtDate(a.workDate)}</td>
                      ...
                      <td style={{ padding: '5px 10px', ...MONO, color: variance > 0 ? 'var(--danger, #f87171)' : variance < 0 ? 'var(--success, #4ade80)' : 'var(--w30, rgba(255,255,255,0.3))', textAlign: 'right' }}>{a.hoursPlanned != null ? `${variance > 0 ? '+' : ''}${variance}h` : '—'}</td>
                      ...
                      <td style={{ padding: '5px 10px', fontSize: 11, color: 'var(--w30, rgba(255,255,255,0.3))' }}>{a.notes || '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10, flexWrap: 'wrap', gap: 10 }}>
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {[
                ...
                { label: 'Over-consumed',   value: String(materialData.summary.overConsumed),      color: materialData.summary.overConsumed > 0 ? 'var(--danger, #f87171)' : 'var(--w30, rgba(255,255,255,0.3))' },
                { label: 'Under-consumed',  value: String(materialData.summary.underConsumed),     color: materialData.summary.underConsumed > 0 ? 'var(--success, #4ade80)' : 'var(--w30, rgba(255,255,255,0.3))' },
                { label: 'Total Var. Cost', value: fmtAmt(materialData.summary.totalVarianceCost), color: materialData.summary.totalVarianceCost > 0 ? 'var(--danger, #f87171)' : materialData.summary.totalVarianceCost < 0 ? 'var(--success, #4ade80)' : 'var(--w30, rgba(255,255,255,0.3))' },
              ].map(s => (
                <div key={s.label} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <span style={{ fontSize: 10, color: 'var(--w30, rgba(255,255,255,0.3))', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{s.label}</span>
                  <span style={{ ...MONO, color: s.color, fontSize: 13 }}>{s.value}</span>
          ...
          {bomSuggestions && bomSuggestions.length > 0 && (
            <div style={{ background: 'rgba(251,146,60,0.05)', border: '0.5px solid rgba(251,146,60,0.2)', borderRadius: 8, padding: '10px 14px', marginBottom: 12 }}>
                ...
                <thead>
                  <tr>
                    ...
                    {['Item Code', 'Item Name', 'Qty Planned', 'UOM', 'Note'].map(h => (
                      <th key={h} style={{ padding: '3px 8px', fontSize: 10, color: 'rgba(251,146,60,0.5)', fontWeight: 500, textTransform: 'uppercase', textAlign: 'left', borderBottom: '0.5px solid var(--l04, rgba(255,255,255,0.04))' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {bomSuggestions.map((s: any) => {
                    ...
                    return (
                      <tr key={s.consumptionGroupId ?? s.itemId ?? s.consumptionGroupCode} style={{ opacity: checked ? 1 : 0.4, cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '5px 8px', fontSize: 12, color: 'var(--w45, rgba(255,255,255,0.45))' }}>{s.uom}</td>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--w25, rgba(255,255,255,0.25))' }}>{bomSelected.size} of {bomSuggestions.length} selected</span>
              </div>
            </div>
          )}
          {bomSuggestions && bomSuggestions.length === 0 && (
          ...
          {materialData.actuals.length === 0 ? (
            <div style={{ fontSize: 12, color: 'var(--w25, rgba(255,255,255,0.25))' }}>No material actuals recorded yet.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                  ...
                  return (
                    <tr key={a.id}>
                      <td style={{ padding: '5px 10px' }}>
                        ...
                        <span style={{ fontSize: 12, color: 'var(--w60, rgba(255,255,255,0.6))', marginLeft: 8 }}>{a.item?.name}</span>
                      </td>
                      ...
                      <td style={{ padding: '5px 10px', ...MONO, color: varQty > 0 ? 'var(--danger, #f87171)' : varQty < 0 ? 'var(--success, #4ade80)' : 'var(--w30, rgba(255,255,255,0.3))', textAlign: 'right' }}>{varQty > 0 ? '+' : ''}{fmtNum(varQty)}</td>
                      <td style={{ padding: '5px 10px', ...MONO, textAlign: 'right' }}>{fmtAmt(a.unitCost)}</td>
                      <td style={{ padding: '5px 10px', ...MONO, color: a.varianceCost > 0 ? 'var(--danger, #f87171)' : a.varianceCost < 0 ? 'var(--success, #4ade80)' : 'var(--w30, rgba(255,255,255,0.3))', textAlign: 'right' }}>{fmtAmt(a.varianceCost)}</td>
                      <td style={{ padding: '5px 10px', fontSize: 11, color: 'var(--w30, rgba(255,255,255,0.3))' }}>{a.notes || '—'}</td>
                    </tr>
                  );
                })}
        ...
        <>
          <div style={{ display: 'flex', gap: 16, marginBottom: 10, flexWrap: 'wrap' }}>
            {[
              { label: 'Open',          value: String(varianceData.summary.open),              color: varianceData.summary.open > 0 ? 'var(--warning, #fbbf24)' : 'var(--w30, rgba(255,255,255,0.3))' },
          ...
          {varianceData.variances.length === 0 ? (
            <div style={{ fontSize: 12, color: 'var(--w25, rgba(255,255,255,0.25))' }}>No variances for this order.</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr>{['Type', 'Description', 'Quantity', 'Unit Cost', 'Total Cost', 'Status', ''].map(h => (
                  <th key={h} style={{ padding: '4px 10px', fontSize: 10, color: 'rgba(248,113,113,0.5)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: 'left', borderBottom: '0.5px solid var(--l04, rgba(255,255,255,0.04))' }}>{h}</th>
                ))}</tr>
              </thead>
              <tbody>
                {varianceData.variances.map((v: Variance) => (
                  <tr key={v.id}>
                    ...
                    <td style={{ padding: '5px 10px', fontSize: 12, color: 'var(--w50, rgba(255,255,255,0.5))' }}>{v.description || '—'}</td>
...
function moColumns(opts: {
  ...
  return [
    ...
    },
    {
      ...
      render: r => <span style={{ fontSize: 12, color: 'var(--w55, rgba(255,255,255,0.55))' }}>{boms.find(b => b.id === r.bomId)?.bomNumber ?? '—'}</span>,
    },
    {
      ...
      render: r => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ width: 52, height: 4, borderRadius: 2, background: 'var(--w10, rgba(255,255,255,0.1))', flexShrink: 0 }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--w30, rgba(255,255,255,0.3))' }}>{pct}%</span>
          </div>
        );
      },
    },
    {
      ...
      render: r => <span style={{ fontSize: 12, color: 'var(--w45, rgba(255,255,255,0.45))' }}>{fmtDate(r.plannedStartDate)}</span>,
    },
    {
      key: 'plannedEndDate', header: 'Planned End', width: 120, sortable: true,
      value: r => r.plannedEndDate ?? '',
      render: r => <span style={{ fontSize: 12, color: 'var(--w45, rgba(255,255,255,0.45))' }}>{fmtDate(r.plannedEndDate)}</span>,
    },
    {
      key: 'priority', header: 'Priority', width: 90, sortable: true,
      value: r => r.priority ?? '',
      render: r => r.priority ? <span style={{ fontSize: 11, color: PRIORITY_COLOR[r.priority] }}>{r.priority.charAt(0).toUpperCase() + r.priority.slice(1)}</span> : <span style={{ color: 'var(--w25, rgba(255,255,255,0.25))' }}>—</span>,
    },
    {
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Manufacturing', 'Production Orders']} title="Production Orders">
      <style>{`
        ...
        .mo-filter { background:var(--l04, rgba(255,255,255,0.04)); border:0.5px solid var(--l09, rgba(255,255,255,0.09)); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-primary, #e2dfd8); outline:none; }
        ...
        .mo-table thead th { padding:9px 14px; font-size:10px; font-weight:500; letter-spacing:0.1em; text-transform:uppercase; color:rgba(251,146,60,0.55); background:rgba(251,146,60,0.05); border-bottom:0.5px solid var(--l06, rgba(255,255,255,0.06)); text-align:left; white-space:nowrap; }
        .mo-table tbody td { padding:10px 14px; border-bottom:0.5px solid var(--l04, rgba(255,255,255,0.04)); vertical-align:middle; font-size:13px; }
        .mo-table tbody tr:last-child td { border-bottom:none; }
        .mo-table tbody tr:hover td { background:rgba(251,146,60,0.03); }
        .mo-empty, .mo-loading { text-align:center; padding:52px 24px; color:var(--w25, rgba(255,255,255,0.25)); font-size:13px; }
        .mo-footer { font-size:11px; color:var(--w22, rgba(255,255,255,0.22)); padding:8px 14px; border-top:0.5px solid var(--l04, rgba(255,255,255,0.04)); }
        ...
        {list.length > 0 && (
          <div className="mo-stats">
            {(Object.entries(PO_STATUS) as [ProductionOrderStatus, typeof PO_STATUS[ProductionOrderStatus]][]).map(([s, style]) => (
              <div key={s} className="mo-stat" style={{ border: `0.5px solid ${statusFilter === s ? style.border : 'var(--l07, rgba(255,255,255,0.07))'}` }} onClick={() => setStatusFilter(prev => prev === s ? '' : s)}>
            ...
            <div className="mo-stat" style={{ border: `0.5px solid ${!statusFilter ? 'rgba(251,146,60,0.3)' : 'var(--l07, rgba(255,255,255,0.07))'}` }} onClick={() => setStatusFilter('')}>
