...
function WCModal({ wc, onClose, onSaved }: { wc: WorkCenter | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'var(--surface, #0e0b1a)', border:'0.5px solid rgba(251,146,60,0.2)', borderRadius:14, width:'100%', maxWidth:520, maxHeight:'92vh', overflowY:'auto', position:'relative', boxShadow:'0 24px 60px rgba(0,0,0,0.7)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 20px 12px', borderBottom:'0.5px solid rgba(255,255,255,0.06)', position:'sticky', top:0, background:'var(--surface, #0e0b1a)', zIndex:1 }}>
          <span style={{ fontSize:14, fontWeight:500, color:'var(--text-strong, #f1ede8)', fontFamily:"'IBM Plex Sans',sans-serif" }}>{wc ? 'Edit Work Center' : 'New Work Center'}</span>
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Manufacturing', 'Work Centers']} title="Work Centers">
      <style>{`
        ...
        .wc-search { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-primary, #e2dfd8); outline:none; width:240px; }
        .wc-search::placeholder { color:rgba(255,255,255,0.2); }
        .wc-search:focus { border-color:rgba(251,146,60,0.4); }
        .wc-btn-new { display:flex; align-items:center; gap:6px; margin-left:auto; background:linear-gradient(135deg,var(--accent-pressed, #c2410c),var(--accent, #ea580c),var(--accent-mid, #f97316)); border:none; border-radius:7px; padding:7px 14px; font-size:12px; font-weight:500; font-family:'IBM Plex Sans',sans-serif; color:white; cursor:pointer; box-shadow:0 3px 12px rgba(234,88,12,0.3); transition:opacity 0.15s; flex-shrink:0; }
        ...
        .wc-error { background:rgba(239,68,68,0.08); border:0.5px solid rgba(239,68,68,0.2); border-radius:8px; padding:10px 14px; margin-bottom:14px; font-size:13px; color:var(--danger-subtle, #fca5a5); }
      `}</style>
          ...
          : (<>
            <table className="wc-table">
              ...
              <tbody>
                {filtered.map(wc => (
                  <tr key={wc.id}>
                    <td><span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:12, color:'var(--accent-strong, #fb923c)' }}>{wc.code}</span></td>
                    <td><span style={{ color:'var(--text-primary, #e2dfd8)', fontWeight:500 }}>{wc.name}</span></td>
                    <td>{wc.workCenterType && <span style={{ fontSize:11, padding:'2px 8px', borderRadius:20, background:'rgba(255,255,255,0.06)', border:'0.5px solid rgba(255,255,255,0.1)', color: WC_TYPE_COLOR[wc.workCenterType] ?? 'var(--text-primary, #e2dfd8)' }}>{wc.workCenterType.charAt(0).toUpperCase()+wc.workCenterType.slice(1)}</span>}</td>
                    ...
                    <td><span style={{ fontSize:11, padding:'2px 8px', borderRadius:20, color: wc.isActive ? 'var(--success, #4ade80)' : 'var(--danger, #f87171)', background: wc.isActive ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)', border:`0.5px solid ${wc.isActive ? 'rgba(74,222,128,0.2)' : 'rgba(248,113,113,0.2)'}` }}>{wc.isActive ? 'Active' : 'Inactive'}</span></td>
                    <td><div style={{ display:'flex', gap:5 }}>
                      ...
                      <button onClick={() => setDeleting(wc)} style={{ padding:'4px 9px', borderRadius:6, fontSize:11, cursor:'pointer', background:'rgba(239,68,68,0.08)', color:'var(--danger, #f87171)', border:'0.5px solid rgba(239,68,68,0.2)', fontFamily:"'IBM Plex Sans',sans-serif" }}>Delete</button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
      ...
      {deleting && (
        <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
          <div style={{ background:'var(--surface, #0e0b1a)', border:'0.5px solid rgba(239,68,68,0.25)', borderRadius:14, width:'100%', maxWidth:400, padding:'24px', boxShadow:'0 24px 60px rgba(0,0,0,0.7)' }}>
            <div style={{ fontSize:14, fontWeight:500, color:'var(--text-strong, #f1ede8)', marginBottom:10 }}>Delete work center?</div>
            <div style={{ fontSize:13, color:'rgba(255,255,255,0.5)', marginBottom:20 }}><strong style={{ color:'var(--text-strong, #f1ede8)' }}>{deleting.name}</strong> will be deleted.</div>
            <div style={{ display:'flex', justifyContent:'flex-end', gap:8 }}>
              ...
              <button onClick={handleDelete} disabled={deleteBusy} style={{ background:'rgba(239,68,68,0.15)', border:'0.5px solid rgba(239,68,68,0.35)', borderRadius:7, padding:'8px 16px', fontSize:13, fontWeight:500, fontFamily:"'IBM Plex Sans',sans-serif", color:'var(--danger, #f87171)', cursor:deleteBusy?'not-allowed':'pointer', opacity:deleteBusy?0.5:1 }}>{deleteBusy?'Deleting…':'Delete'}</button>
            </div>
          </div>
        </div>
      )}
