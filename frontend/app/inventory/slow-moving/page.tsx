...
const RISK_CFG: Record<RiskLevel, { color: string; bg: string; border: string; label: string }> = {
  dead:     { color: 'var(--danger, #f87171)', bg: 'rgba(248,113,113,0.15)', border: 'rgba(248,113,113,0.3)', label: 'Dead Stock'    },
  critical: { color: 'var(--accent-strong, #fb923c)', bg: 'rgba(251,146,60,0.12)',  border: 'rgba(251,146,60,0.25)', label: 'Critical Slow' },
  slow:     { color: 'var(--warning, #fbbf24)', bg: 'rgba(251,191,36,0.12)',  border: 'rgba(251,191,36,0.25)', label: 'Slow Moving'   },
  watch:    { color: 'var(--accent-violet, #a78bfa)', bg: 'rgba(167,139,250,0.1)',  border: 'rgba(167,139,250,0.2)', label: 'Watch'         },
};
  ...
  },
  {
    ...
    render: r => {
      ...
      return r.daysSinceLastMovement !== null
        ...
        : <span style={{ fontSize: 10, color: 'var(--danger, #f87171)' }}>Never</span>;
    },
  },
  {
    ...
    <ERPShell breadcrumbs={['Home', 'Inventory', 'Slow Moving & Dead Stock']} title="Slow Moving & Dead Stock">
      ...
      <div className="sm-page">
        ...
        <div className="sm-kpis">
          {[
            { label: 'Items at Risk',   value: String(slowRows.length),                   color: 'var(--danger, #f87171)', border: 'rgba(248,113,113,0.2)'  },
            { label: 'Capital Tied Up', value: fmtAmt(totalAtRisk),                       color: 'var(--danger, #f87171)', border: 'rgba(248,113,113,0.2)'  },
            { label: '% of Inventory',  value: atRiskPct + '%',                           color: 'var(--accent-strong, #fb923c)', border: 'rgba(251,146,60,0.2)'   },
            { label: 'Dead Stock Items',value: String(data?.summary.deadStockCount ?? 0), color: 'var(--danger, #f87171)', border: 'rgba(248,113,113,0.15)' },
            { label: 'Dead Stock Value',value: fmtAmt(data?.summary.deadStockValue ?? 0), color: 'var(--danger, #f87171)', border: 'rgba(248,113,113,0.15)' },
          ].map(k => (
        ...
        {(data?.summary.deadStockCount ?? 0) > 0 && !cardFilter && (
          <div className="sm-alert-banner">
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--danger, #f87171)', flexShrink: 0 }} />
            <div>
