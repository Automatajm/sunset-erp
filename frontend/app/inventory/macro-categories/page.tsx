...
function Modal({ open, onClose, onSaved, initial }: {
  ...
  return (
    <>
      <style>{`.mc-overlay{position:fixed;inset:0;z-index:400;background:rgba(0,0,0,0.65);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:24px}.mc-box{background:var(--surface);border:0.5px solid rgba(251,146,60,0.2);border-radius:14px;width:100%;max-width:460px;box-shadow:0 24px 60px rgba(0,0,0,0.7)}.mc-hdr{display:flex;align-items:center;justify-content:space-between;padding:14px 18px 12px;border-bottom:0.5px solid rgba(255,255,255,0.06)}.mc-title{font-size:13px;font-weight:500;color:var(--text-strong)}.mc-close{width:22px;height:22px;border-radius:6px;background:rgba(255,255,255,0.06);border:none;cursor:pointer;color:rgba(255,255,255,0.4);font-size:15px;display:flex;align-items:center;justify-content:center}.mc-body{padding:14px 18px;display:flex;flex-direction:column;gap:10px}.mc-field{display:flex;flex-direction:column;gap:5px}.mc-label{font-size:11px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;color:rgba(251,146,60,0.55)}.mc-input,.mc-textarea{background:rgba(255,255,255,0.04);border:0.5px solid rgba(255,255,255,0.1);border-radius:7px;padding:8px 12px;font-size:13px;font-family:'IBM Plex Sans',sans-serif;color:var(--text-strong);outline:none;width:100%}.mc-input:focus,.mc-textarea:focus{border-color:rgba(251,146,60,0.45)}.mc-textarea{resize:vertical;min-height:60px}.mc-error{background:rgba(239,68,68,0.1);border:0.5px solid rgba(239,68,68,0.25);border-radius:7px;padding:7px 12px;font-size:12px;color:var(--danger-subtle)}.mc-ftr{display:flex;justify-content:flex-end;gap:8px;padding:10px 18px 16px;border-top:0.5px solid rgba(255,255,255,0.06)}.mc-btn-cancel{background:rgba(255,255,255,0.05);border:0.5px solid rgba(255,255,255,0.1);border-radius:7px;padding:7px 14px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:rgba(255,255,255,0.45);cursor:pointer}.mc-btn-save{background:linear-gradient(135deg,var(--accent-pressed),var(--accent),var(--accent-mid));border:none;border-radius:7px;padding:7px 18px;font-size:12px;font-weight:500;font-family:'IBM Plex Sans',sans-serif;color:white;cursor:pointer}.mc-btn-save:disabled{opacity:0.5;cursor:not-allowed}.mc-row{display:grid;grid-template-columns:1fr 1fr;gap:10px}`}</style>
    ...
    <ERPShell breadcrumbs={['Home', 'Inventory', 'Macro Categories']} title="Macro Categories">
                ...
                <tbody>
                  {filtered.map(mc => (
                    <tr key={mc.id}>
                      ...
                      <td style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{mc.name}</td>
                      <td><span className="mcp-muted">{mc.description || '—'}</span></td>
                      <td><span className="mcp-count">{mc._count?.categories ?? 0} categories</span></td>
                      <td>
                        <span style={{ color: mc.isActive ? 'var(--success)' : 'rgba(255,255,255,0.3)', fontSize: 12 }}>
      ...
      {deleting && (
        <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
          <div style={{ background:'var(--surface)', border:'0.5px solid rgba(239,68,68,0.25)', borderRadius:14, width:'100%', maxWidth:380, padding:'22px 22px 18px', boxShadow:'0 24px 60px rgba(0,0,0,0.7)' }}>
            <div style={{ fontSize:14, fontWeight:500, color:'var(--text-strong)', marginBottom:10 }}>Delete macro category?</div>
            <div style={{ fontSize:13, color:'rgba(255,255,255,0.45)', marginBottom:20, lineHeight:1.5 }}>
              <strong style={{ color:'var(--text-strong)' }}>{deleting.name}</strong> ({deleting.code}) will be soft-deleted.
              {(deleting._count?.categories ?? 0) > 0 && <span style={{ color:'var(--danger-subtle)' }}> This macro category has {deleting._count?.categories} categories — delete them first.</span>}
            </div>
