  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 2000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#e2dfd8', fontFamily: "'IBM Plex Sans',sans-serif" }}>
          Scan Item Barcode
        </div>
        <button
          onClick={onClose}
      ...
      {!error ? (
        <div style={{ position: 'relative', width: '100%', maxWidth: 360, aspectRatio: '1', overflow: 'hidden', borderRadius: 16 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ position: 'relative', width: '70%', height: '70%' }}>
              {/* Corners */}
              {[
                { top: 0, left: 0, borderTop: '3px solid #fb923c', borderLeft: '3px solid #fb923c', borderRadius: '8px 0 0 0' },
                { top: 0, right: 0, borderTop: '3px solid #fb923c', borderRight: '3px solid #fb923c', borderRadius: '0 8px 0 0' },
                { bottom: 0, left: 0, borderBottom: '3px solid #fb923c', borderLeft: '3px solid #fb923c', borderRadius: '0 0 0 8px' },
                { bottom: 0, right: 0, borderBottom: '3px solid #fb923c', borderRight: '3px solid #fb923c', borderRadius: '0 0 8px 0' },
              ].map((style, i) => (
              ...
              {scanning && (
                <div style={{ position: 'absolute', left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, transparent, #fb923c, transparent)', animation: 'scan-line 1.5s ease-in-out infinite alternate', boxShadow: '0 0 8px #fb923c' }} />
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.3)', borderRadius: 12, padding: '20px 24px', maxWidth: 320, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: '#fca5a5', fontFamily: "'IBM Plex Sans',sans-serif", lineHeight: 1.5 }}>{error}</div>
        </div>
      )}
