...
const ITEM_TYPE_CFG: Record<string, { color: string; label: string }> = {
  ...
  service:       { color: '#a78bfa', label: 'Service'       },
};
...
function UomToggle({ active, onChange }: { active: UomView; onChange: (v: UomView) => void }) {
  return (
    <div style={{ display: 'flex', gap: 0, background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 8, overflow: 'hidden', flexShrink: 0 }}>
      {UOM_VIEWS.map((v, i) => (
        <button key={v.value} type="button" onClick={() => onChange(v.value)}
          ...
          style={{
            ...
            color: active === v.value ? '#fb923c' : 'rgba(255,255,255,0.35)',
...
function StockDrawer({ row, onClose }: { row: StockBalance | null; onClose: () => void }) {
  if (!row) return null;
  const itCfg = ITEM_TYPE_CFG[row.item?.itemType ?? ''] ?? { color: 'rgba(255,255,255,0.35)', label: row.item?.itemType ?? '' };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, display: 'flex' }}
      ...
      <div style={{ width: 380, background: '#0e0b1a', borderLeft: '0.5px solid rgba(251,146,60,0.2)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}
        ...
        <div style={{ padding: '18px 20px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ ...MONO, fontSize: 15, fontWeight: 500, color: '#fb923c' }}>{row.item?.code}</div>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, color: itCfg.color, background: `${itCfg.color}15`, border: `0.5px solid ${itCfg.color}30` }}>
              {itCfg.label}
            </span>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {sections.map(sec => (
            <div key={sec.title} style={{ background: 'rgba(255,255,255,0.02)', border: `0.5px solid ${sec.color}20`, borderRadius: 10, overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px 8px', borderBottom: `0.5px solid ${sec.color}15` }}>
                <div style={{ fontSize: 11, fontWeight: 500, color: sec.color }}>{sec.title}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>{sec.subtitle}</div>
              </div>
              <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 7 }}>
                {sec.rows.map(r => (
                  <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{r.label}</span>
                    <span style={{ ...MONO, fontSize: 12, fontWeight: r.highlight ? 600 : 400, color: r.highlight ? sec.color : '#f1ede8' }}>{r.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {(row.lotNumber || row.serialNumber) && (
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 7 }}>
              ...
              {row.lotNumber    && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Lot</span><span style={{ ...MONO, fontSize: 12, color: '#fbbf24' }}>{row.lotNumber}</span></div>}
              {row.serialNumber && <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Serial</span><span style={{ ...MONO, fontSize: 12, color: '#fbbf24' }}>{row.serialNumber}</span></div>}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Inventory', 'Stock Balance']} title="Stock Balance">
      <style>{`
        ...
        .sb-search{background:rgba(255,255,255,0.04);border:0.5px solid rgba(255,255,255,0.09);border-radius:7px;padding:7px 12px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:#e2dfd8;outline:none;width:220px}
        ...
        .sb-table thead th{padding:8px 12px;font-size:9px;font-weight:500;letter-spacing:0.1em;text-transform:uppercase;color:rgba(251,146,60,0.55);background:rgba(251,146,60,0.05);border-bottom:0.5px solid rgba(255,255,255,0.06);text-align:left;white-space:nowrap;position:sticky;top:0;z-index:2}
        ...
        .sb-table tbody td{padding:10px 12px;border-bottom:0.5px solid rgba(255,255,255,0.03);vertical-align:middle}
        ...
        .sb-spinner{width:18px;height:18px;border-radius:50%;border:2px solid rgba(251,146,60,0.2);border-top-color:#fb923c;animation:sb-spin 0.7s linear infinite}
        ...
        .sb-error{background:rgba(239,68,68,0.08);border:0.5px solid rgba(239,68,68,0.2);border-radius:8px;padding:10px 14px;margin-bottom:10px;font-size:13px;color:#fca5a5;flex-shrink:0}
      ...
      <div className="sb-page">
        ...
        <div className="sb-kpis">
          {[
            { label: 'Total Value',   value: fmtAmt(totalValue),           color: '#fb923c', border: 'rgba(251,146,60,0.2)'   },
            { label: 'Unique Items',  value: String(totalItems),            color: '#60a5fa', border: 'rgba(96,165,250,0.15)'  },
            { label: 'Warehouses',    value: String(totalWH),               color: '#a78bfa', border: 'rgba(167,139,250,0.15)' },
            { label: 'Low Stock',     value: String(lowStockCount),         color: lowStockCount > 0 ? '#fbbf24' : 'rgba(255,255,255,0.3)', border: lowStockCount > 0 ? 'rgba(251,191,36,0.2)' : 'rgba(255,255,255,0.06)' },
            { label: 'Zero Stock',    value: String(zeroCount),             color: zeroCount  > 0 ? '#f87171' : 'rgba(255,255,255,0.3)',   border: zeroCount  > 0 ? 'rgba(248,113,113,0.2)' : 'rgba(255,255,255,0.06)' },
          ].map(k => (
        ...
        <div style={{ marginBottom: 8, flexShrink: 0 }}>
          {UOM_VIEWS.filter(v => v.value === uomView).map(v => (
            <div key={v.value} style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ color: uomView === 'purchase' ? '#fb923c' : uomView === 'storage' ? '#60a5fa' : '#4ade80' }}>
                {v.icon} {v.label}
              </span>
              <span>—</span>
              <span>{v.desc}</span>
            ...
            <>
              <div className="sb-scroll">
                <table className="sb-table">
                  ...
                  <tbody>
                    {filtered.map(row => {
                      const itCfg = ITEM_TYPE_CFG[row.item?.itemType ?? ''] ?? { color: 'rgba(255,255,255,0.35)', label: '' };
                      ...
                      return (
                        <tr key={row.id} onClick={() => setSelected(row)}
                          ...
                          {/* Item */}
                          <td>
                            <div style={{ ...MONO, fontSize: 12, color: '#fb923c', fontWeight: 500 }}>{row.item?.code ?? '—'}</div>
                          ...
                          <td>
                            {row.item?.itemType && (
                              <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 10, color: itCfg.color, background: `${itCfg.color}15`, border: `0.5px solid ${itCfg.color}30`, whiteSpace: 'nowrap' }}>
                                {itCfg.label}
                              </span>
                            )}
                          </td>
                          ...
                          {/* Warehouse */}
                          <td>
                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>{row.warehouse?.code}</div>
                          ...
                          {uomView === 'purchase' && <>
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 13, fontWeight: 500, color: isZero ? 'rgba(255,255,255,0.2)' : '#fb923c' }}>
                                {fmtQty(row.purchaseQty)}
                              </span>
                            </td>
                            <td>
                              <span className="sb-uom-badge" style={{ color: '#fb923c', background: 'rgba(251,146,60,0.1)', border: '0.5px solid rgba(251,146,60,0.2)' }}>
                                {row.purchaseUom}
                              </span>
                            </td>
                            <td style={{ textAlign: 'right' }}>
                            ...
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 13, fontWeight: 600, color: isZero ? 'rgba(255,255,255,0.2)' : '#4ade80' }}>
                          ...
                          {uomView === 'storage' && <>
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 13, fontWeight: 500, color: isZero ? 'rgba(255,255,255,0.2)' : '#60a5fa' }}>
                                {fmtQty(row.storageQty)}
                              </span>
                            </td>
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 12, color: row.reservedQuantity > 0 ? '#fbbf24' : 'rgba(255,255,255,0.3)' }}>
                                {fmtQty(row.reservedQuantity)}
                              </span>
                            </td>
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 13, fontWeight: 600, color: isZero ? '#f87171' : isLow ? '#fbbf24' : '#4ade80' }}>
                                {fmtQty(row.availableQty)}
                              </span>
                              {isLow && !isZero && <div style={{ fontSize: 9, color: '#fbbf24', marginTop: 1 }}>⚠ low</div>}
                            </td>
                            <td>
                              <span className="sb-uom-badge" style={{ color: '#60a5fa', background: 'rgba(96,165,250,0.1)', border: '0.5px solid rgba(96,165,250,0.2)' }}>
                                {row.storageUom}
                              </span>
                            </td>
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{row.unitCostStorage > 0 ? fmtAmt(row.unitCostStorage) : '—'}</span>
                            </td>
                          </>}
                          ...
                          {uomView === 'consumption' && <>
                            <td style={{ textAlign: 'right' }}>
                              <span style={{ ...MONO, fontSize: 13, fontWeight: 500, color: isZero ? 'rgba(255,255,255,0.2)' : '#4ade80' }}>
                                {fmtQty(row.consumptionQty)}
                              </span>
                            </td>
                            <td>
                              <span className="sb-uom-badge" style={{ color: '#4ade80', background: 'rgba(74,222,128,0.1)', border: '0.5px solid rgba(74,222,128,0.2)' }}>
                                {row.consumptionUom}
                              </span>
                            </td>
                            <td style={{ textAlign: 'right' }}>
                          ...
                          <td>
                            {row.lotNumber
                              ? <span style={{ ...MONO, fontSize: 10, color: '#fbbf24' }}>{row.lotNumber}</span>
              ...
              <div className="sb-footer">
                ...
                <span style={{ ...MONO, color: '#fb923c', fontSize: 12 }}>{fmtAmt(totalValue)} total inventory value</span>
