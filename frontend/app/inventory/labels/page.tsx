...
function HelpModal({ sz, onClose }: { sz: SizeConfig; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#0a0712', border: '0.5px solid rgba(96,165,250,0.2)', borderRadius: 14, width: '100%', maxWidth: 480, maxHeight: '85vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.8)' }}>
        <div style={{ padding: '16px 20px 12px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#e2dfd8' }}>How to Print Labels</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14, fontFamily: "'IBM Plex Sans',sans-serif" }}>
          <div style={{ background: 'rgba(251,146,60,0.06)', border: '0.5px solid rgba(251,146,60,0.2)', borderRadius: 8, padding: '10px 14px' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#fb923c', marginBottom: 4 }}>Selected: {sz.label} - {sz.desc}</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{sz.wMm.toFixed(1)} x {sz.hMm.toFixed(1)} mm</div>
          </div>
          {steps.map(s => (
            <div key={s.n} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(251,146,60,0.15)', border: '0.5px solid rgba(251,146,60,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#fb923c', flexShrink: 0 }}>{s.n}</div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#e2dfd8', marginBottom: 2 }}>{s.title}</div>
...
const INP:  React.CSSProperties = { background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 7, padding: '7px 10px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: '#e2dfd8', outline: 'none', width: '100%', boxSizing: 'border-box' as const };
...
const CHIP  = (active: boolean, color = '#fb923c'): React.CSSProperties => ({ fontSize: 10, padding: '3px 9px', borderRadius: 20, border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif", fontWeight: 500, background: active ? `${color}22` : 'rgba(255,255,255,0.05)', color: active ? color : 'rgba(255,255,255,0.4)', outline: active ? `1px solid ${color}44` : 'none' });
...
export default function LabelPrintPage() {
  ...
  return (
    <>
      ...
      {/* Screen UI */}
      <div className="screen-ui" style={{ height: '100vh', background: '#06040f', color: '#e2dfd8', fontFamily: "'IBM Plex Sans',sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 24px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={() => setShowHelp(true)} title="How to print labels" style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(96,165,250,0.08)', border: '0.5px solid rgba(96,165,250,0.2)', fontSize: 14, fontWeight: 700, color: '#60a5fa', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>?</button>
            <button onClick={() => window.print()} disabled={selCount === 0} style={{ background: selCount > 0 ? 'linear-gradient(135deg,#c2410c,#ea580c,#f97316)' : 'rgba(255,255,255,0.05)', border: 'none', borderRadius: 9, padding: '10px 24px', fontSize: 14, fontWeight: 600, color: selCount > 0 ? 'white' : 'rgba(255,255,255,0.2)', cursor: selCount > 0 ? 'pointer' : 'not-allowed', fontFamily: "'IBM Plex Sans',sans-serif" }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {(['internal','external','both'] as BarcodeMode[]).map(m => (
                    <button key={m} onClick={() => setBarcodeMode(m)} style={{ flex: 1, height: 30, borderRadius: 6, fontSize: 11, fontWeight: 500, border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif", textTransform: 'capitalize' as const, background: barcodeMode === m ? 'rgba(96,165,250,0.15)' : 'rgba(255,255,255,0.05)', color: barcodeMode === m ? '#60a5fa' : 'rgba(255,255,255,0.4)', outline: barcodeMode === m ? '1px solid rgba(96,165,250,0.3)' : 'none' }}>
                      {m}
                    </button>
                  ))}
                </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {(['mm','cm','in'] as Unit[]).map(u => (
                  <button key={u} onClick={() => setUnit(u)} style={{ flex: 1, height: 28, borderRadius: 6, fontSize: 11, fontWeight: 500, border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif", background: unit === u ? 'rgba(251,146,60,0.15)' : 'rgba(255,255,255,0.05)', color: unit === u ? '#fb923c' : 'rgba(255,255,255,0.4)', outline: unit === u ? '1px solid rgba(251,146,60,0.3)' : 'none', textTransform: 'uppercase' as const }}>
                    {u}
                  </button>
                ))}
              </div>
            ...
            {labelType === 'item' && (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => setSelItems(s => new Set([...s, ...filteredItems.map(i => i.id)]))} style={{ fontSize: 10, color: '#fb923c', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>+ Shown</button>
                    <button onClick={() => setSelItems(new Set(items.map(i => i.id)))} style={{ fontSize: 10, color: '#fb923c', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>All</button>
                ...
                <div style={{ flex: 1, maxHeight: 260, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {filteredItems.map(item => (
                    <label key={item.id} style={SELROW(selItems.has(item.id))}>
                      <input type="checkbox" checked={selItems.has(item.id)} onChange={() => setSelItems(toggle(selItems, item.id))} style={{ accentColor: '#fb923c' }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                          <span style={{ ...MONO, fontSize: 11, color: '#fb923c' }}>{item.code}</span>
                          {item.barcodeExternal && (
                            <span style={{ fontSize: 8, padding: '1px 4px', borderRadius: 8, background: 'rgba(96,165,250,0.1)', color: '#60a5fa', fontFamily: "'IBM Plex Sans',sans-serif" }}>EXT</span>
                          )}
                        </div>
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      {(['all','levels','bins'] as LocFilter[]).map(f => (
                        <button key={f} onClick={() => setLocFilter(f)} style={{ flex: 1, height: 28, borderRadius: 6, fontSize: 10, fontWeight: 500, border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif", background: locFilter === f ? 'rgba(96,165,250,0.15)' : 'rgba(255,255,255,0.05)', color: locFilter === f ? '#60a5fa' : 'rgba(255,255,255,0.4)', outline: locFilter === f ? '1px solid rgba(96,165,250,0.3)' : 'none', textTransform: 'capitalize' as const }}>
                          {f}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => setSelLocs(s => new Set([...s, ...filteredLocs.map(l => l.id)]))} style={{ fontSize: 10, color: '#60a5fa', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>+ Shown</button>
                        <button onClick={() => setSelLocs(new Set(locations.map(l => l.id)))} style={{ fontSize: 10, color: '#60a5fa', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>All</button>
                    ...
                    <div style={{ flex: 1, maxHeight: 300, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
                      {loading ? (
                        ...
                        <label key={loc.id} style={LOCROW(selLocs.has(loc.id))}>
                          <input type="checkbox" checked={selLocs.has(loc.id)} onChange={() => setSelLocs(toggle(selLocs, loc.id))} style={{ accentColor: '#60a5fa' }} />
                          <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ ...MONO, fontSize: 11, color: loc.type === 'bin' ? '#e2dfd8' : '#60a5fa' }}>{loc.fullCode}</span>
                            <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 10, background: loc.type === 'bin' ? 'rgba(255,255,255,0.07)' : 'rgba(96,165,250,0.1)', color: loc.type === 'bin' ? 'rgba(255,255,255,0.4)' : '#60a5fa', fontFamily: "'IBM Plex Sans',sans-serif" }}>{loc.type}</span>
                          </div>
                        </label>
                      ))}
                    </div>
          ...
          <div style={{ flex: 1, padding: 16, overflowY: 'auto' }}>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginBottom: 12, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              ...
              {labelType === 'item' && <span style={{ marginLeft: 8, color: '#60a5fa' }}>Barcode: {barcodeMode}</span>}
              {copies > 1 && <span style={{ marginLeft: 8, color: '#fb923c' }}>x{copies} copies</span>}
            </div>
            {selCount === 0 ? (
