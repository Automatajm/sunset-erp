...
// ─── Create Transaction Modal (manual entry only) ─────────────────────────────
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#0e0b1a', border: '0.5px solid rgba(251,146,60,0.2)', borderRadius: 14, width: '100%', maxWidth: 520, maxHeight: '90vh', display: 'flex', flexDirection: 'column', position: 'relative', boxShadow: '0 24px 60px rgba(0,0,0,0.7)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px 12px', flexShrink: 0, borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, color: '#f1ede8', fontFamily: "'IBM Plex Sans', sans-serif" }}>
              Manual Stock Adjustment
            </div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>
              For PO receipts, use the <span style={{ color: '#60a5fa' }}>Goods Receipts</span> module
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          <div style={{ padding: '14px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {error && (
              <div style={{ background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.25)', borderRadius: 7, padding: '8px 12px', fontSize: 12, color: '#fca5a5' }}>
                {error}
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <select style={{ ...I, cursor: 'pointer' }} value={form.itemId}
                onChange={e => {
                  ...
                  setForm(f => ({ ...f, itemId: e.target.value, uom: it?.baseUom ?? f.uom }));
                }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <select style={{ ...I, cursor: 'pointer' }} value={form.warehouseId}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <input style={I} type="number" step="0.0001" placeholder="0.00"
                  value={form.unitCost ?? ''}
                  onChange={e => setForm(f => ({ ...f, unitCost: e.target.value === '' ? undefined : Number(e.target.value) }))} />
              </div>
            </div>

            {/* Negative quantity hint for issues */}
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <textarea
                style={{ ...I, resize: 'vertical', minHeight: 56 } as React.CSSProperties}
                placeholder="Reason for adjustment, cycle count discrepancy, etc."
                value={form.notes}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 20px 16px', borderTop: '0.5px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            ...
            <button type="submit" disabled={busy}
              style={{ background: 'linear-gradient(135deg,#c2410c,#ea580c,#f97316)', border: 'none', borderRadius: 7, padding: '8px 20px', fontSize: 12, fontWeight: 500, fontFamily: "'IBM Plex Sans', sans-serif", color: 'white', cursor: 'pointer', boxShadow: '0 3px 12px rgba(234,88,12,0.35)', opacity: busy ? 0.5 : 1 }}>
...
const COLUMNS: ERPColumn<LedgerRow>[] = [
  ...
  },
  {
    ...
    render: r => {
      const isIn = r.signedQuantity > 0, isOut = r.signedQuantity < 0;
      return (
      ...
      return (
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ marginTop: 2 }}>
            <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 8, color: '#fb923c', background: 'rgba(251,146,60,0.1)', border: '0.5px solid rgba(251,146,60,0.2)', ...MONO }}>
              {pUom}
            </span>
          </div>
        </div>
...
export default function StockTransactionsPage() {
  ...
  // Type pill filter (external — above ERPTable)
  ...
  const totals = useMemo(() => ({
    ...
    totalInValue: filtered.filter(r => r.signedQuantity > 0).reduce((s, r) => s + (r.movementValue ?? r.totalValue), 0),
    totalOutValue:filtered.filter(r => r.signedQuantity < 0).reduce((s, r) => s + Math.abs(r.movementValue ?? r.totalValue), 0),
      ...
      <div className="st-page">
        ...
        {rows.length > 0 && (
          <div className="st-pills">
            ...
            <div className="st-pill"
              style={{ border: `0.5px solid ${!typeCard ? 'rgba(251,146,60,0.3)' : 'rgba(255,255,255,0.07)'}` }}
              onClick={() => setTypeCard('')}>
        ...
        <div className="st-filters">
          <ERPFilterBar
            ...
            onChange={setValue}
            onReset={reset}
            activeCount={activeCount + (typeCard ? 1 : 0)}
          />
