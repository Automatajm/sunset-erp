...
const STATUS_STYLE: Record<InvoiceStatus, { color: string; bg: string; border: string }> = {
  ...
  sent:    { color: '#60a5fa', bg: 'rgba(96,165,250,0.1)',  border: 'rgba(96,165,250,0.2)' },
  partial: { color: '#fb923c', bg: 'rgba(251,146,60,0.1)',  border: 'rgba(251,146,60,0.2)' },
  paid:    { color: '#4ade80', bg: 'rgba(74,222,128,0.1)',  border: 'rgba(74,222,128,0.2)' },
  overdue: { color: '#f87171', bg: 'rgba(248,113,113,0.1)', border: 'rgba(248,113,113,0.2)' },
...
function KpiCards({ kpis, aging }: { kpis: ArKpis | null; aging: ArAging | null }) {
  if (!kpis) return null;
  const cards = [
    { label: 'Total Invoiced',    value: fmtAmt(kpis.invoiced),       color: '#e2dfd8' },
    { label: 'Collected',         value: fmtAmt(kpis.collected),       color: '#4ade80' },
    { label: 'Pending',           value: fmtAmt(kpis.pending),         color: '#60a5fa' },
    { label: 'Overdue',           value: fmtAmt(kpis.overdue),         color: '#f87171' },
    { label: 'Collection Rate',   value: `${kpis.collectionRate.toFixed(1)}%`, color: kpis.collectionRate >= 80 ? '#4ade80' : '#fbbf24' },
  ];
  return (
    <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
      ...
      {aging && (
        <div style={{ background: 'rgba(10,7,18,0.7)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '8px 14px', display: 'flex', gap: 16, alignItems: 'center' }}>
          {[
            { label: 'Current',  val: aging.summary.current.amount,    color: '#4ade80' },
            { label: '1-30d',    val: aging.summary.days1to30.amount,   color: '#fbbf24' },
            { label: '31-60d',   val: aging.summary.days31to60.amount,  color: '#fb923c' },
            { label: '90d+',     val: aging.summary.days90plus.amount,  color: '#f87171' },
          ].map(b => (
      ...
      <tr style={{ cursor: 'pointer' }} onClick={handleExpand}>
        ...
        <td>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ ...MONO, color: '#60a5fa', fontWeight: 500 }}>{inv.invoiceNumber}</span>
          </span>
        </td>
        {/* Customer */}
        <td>
          <span style={{ color: '#e2dfd8', fontWeight: 500 }}>{inv.customer?.name ?? '—'}</span>
        ...
        {/* Due Date */}
        <td>
          <span style={{ fontSize: 12, color: dueSoon ? '#fbbf24' : inv.status === 'overdue' ? '#f87171' : 'rgba(255,255,255,0.5)' }}>
            {fmtDate(inv.dueDate)}
            {dueSoon && <span style={{ marginLeft: 4, fontSize: 10, color: '#fbbf24' }}>({days}d)</span>}
          </span>
        </td>
        {/* Total */}
        ...
        <td style={{ textAlign: 'right', minWidth: 110 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
            <div style={{ width: 44, height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.08)', overflow: 'hidden', flexShrink: 0 }}>
              <div style={{ height: '100%', width: `${Math.min(100, pctPaid)}%`, background: pctPaid >= 100 ? '#4ade80' : '#fb923c', borderRadius: 2, transition: 'width 0.3s' }} />
            </div>
            <span style={{ ...MONO, fontSize: 11, color: pctPaid >= 100 ? '#4ade80' : outstanding > 0 ? '#fb923c' : 'rgba(255,255,255,0.3)' }}>
              {fmtAmt(outstanding)}
            </span>
          </div>
        </td>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {inv.status === 'draft' && (
              <button onClick={() => onAction(inv.id, 'send')} disabled={busy}
                style={{ padding: '3px 9px', borderRadius: 6, fontSize: 11, cursor: 'pointer', color: '#60a5fa', background: 'rgba(96,165,250,0.1)', border: '0.5px solid rgba(96,165,250,0.2)', fontFamily: "'IBM Plex Sans',sans-serif", opacity: busy ? 0.5 : 1 }}>
                {busy ? '…' : 'Send'}
              </button>
            )}
            {['sent', 'partial'].includes(inv.status) && (
              <button onClick={() => onPayment(inv)} disabled={busy}
                style={{ padding: '3px 9px', borderRadius: 6, fontSize: 11, cursor: 'pointer', color: '#4ade80', background: 'rgba(74,222,128,0.1)', border: '0.5px solid rgba(74,222,128,0.2)', fontFamily: "'IBM Plex Sans',sans-serif", opacity: busy ? 0.5 : 1 }}>
                + Payment
              </button>
            )}
            {['draft', 'sent', 'partial'].includes(inv.status) && (
              <button onClick={() => onAction(inv.id, 'void')} disabled={busy}
                style={{ padding: '3px 9px', borderRadius: 6, fontSize: 11, cursor: 'pointer', color: '#f87171', background: 'rgba(248,113,113,0.08)', border: '0.5px solid rgba(248,113,113,0.15)', fontFamily: "'IBM Plex Sans',sans-serif", opacity: busy ? 0.5 : 1 }}>
                Void
              </button>
            )}
                  ...
                  <tbody>
                    {detail.lines?.map(line => (
                      <tr key={line.id}>
                        ...
                        <td style={{ padding: '7px 14px', textAlign: 'right', fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: '#e2dfd8', fontWeight: 500 }}>{fmtAmt(line.lineTotal)}</td>
                    ...
                    <tr style={{ borderTop: '0.5px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)' }}>
                      ...
                      <td style={{ padding: '8px 14px', textAlign: 'right', fontFamily: "'IBM Plex Mono',monospace", color: '#60a5fa', fontWeight: 600, fontSize: 13 }}>{fmtAmt(detail.totalAmount)}</td>
                      <td />
                    </tr>
                  </tbody>
                </table>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {detail.payments.map(p => (
                        <div key={p.id} style={{ display: 'flex', gap: 16, alignItems: 'center', fontSize: 12 }}>
                          <span style={{ fontFamily: "'IBM Plex Mono',monospace", color: '#4ade80', fontWeight: 500 }}>{fmtAmt(p.amount)}</span>
...
function PaymentModal({ inv, onClose, onSaved }: {
  ...
  const inp = { background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 7, padding: '8px 12px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: '#f1ede8', outline: 'none', width: '100%' } as React.CSSProperties;
  ...
  return (
    <>
      ...
      <div className="pay-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        <div style={{ background: '#0e0b1a', border: '0.5px solid rgba(74,222,128,0.2)', borderRadius: 14, width: '100%', maxWidth: 420, boxShadow: '0 24px 60px rgba(0,0,0,0.7)', position: 'relative' }}>
          <div style={{ padding: '14px 18px 10px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 500, color: '#f1ede8' }}>Apply Payment</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>
                {inv.invoiceNumber} · Outstanding: <span style={{ color: '#fb923c', fontFamily: "'IBM Plex Mono',monospace" }}>{fmtAmt(outstanding)}</span>
              </div>
            </div>
  ...
  return (
    <>
      <style>{`
        ...
        .inv-box { background:#0e0b1a; border:0.5px solid rgba(96,165,250,0.2); border-radius:14px; width:100%; max-width:820px; margin:auto; box-shadow:0 24px 60px rgba(0,0,0,0.7); }
        .inv-line-input { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.1); border-radius:5px; padding:5px 7px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:#f1ede8; outline:none; width:100%; }
        .inv-line-select { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.1); border-radius:5px; padding:5px 7px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:#f1ede8; outline:none; width:100%; cursor:pointer; }
        .inv-line-select option { background:#0e0b1a; }
        ...
        .inv-btn-rm { width:20px; height:20px; border-radius:4px; background:rgba(239,68,68,0.1); border:0.5px solid rgba(239,68,68,0.2); color:#f87171; cursor:pointer; font-size:13px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
      `}</style>
      <div className="inv-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="inv-box">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px 10px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, background: '#0e0b1a', zIndex: 1, borderRadius: '14px 14px 0 0' }}>
            <span style={{ fontSize: 14, fontWeight: 500, color: '#f1ede8' }}>New Invoice</span>
            <button onClick={onClose} style={{ width: 24, height: 24, borderRadius: 6, background: 'rgba(255,255,255,0.06)', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.45)', fontSize: 16 }}>×</button>
          </div>
          <form onSubmit={handleSubmit}>
            <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <tbody>
                  {lines.map((line, idx) => (
                    <tr key={idx}>
                      ...
                      <td style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: '#e2dfd8', textAlign: 'right', padding: '4px 6px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 16, padding: '6px 0', borderTop: '0.5px solid rgba(255,255,255,0.06)' }}>
                ...
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 14, fontWeight: 500, color: '#60a5fa' }}>{fmtAmt(grandTotal)}</span>
              </div>
            </div>
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Sales', 'AR Invoices']} title="Accounts Receivable">
      <style>{`
        ...
        .ar-stat-value { font-size:18px; font-weight:500; font-family:'IBM Plex Mono',monospace; color:#f1ede8; }
        .ar-toolbar { display:flex; align-items:center; gap:10px; margin-bottom:14px; flex-wrap:wrap; }
        .ar-search { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:#e2dfd8; outline:none; width:240px; }
        ...
        .ar-filter { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:#e2dfd8; outline:none; cursor:pointer; }
        .ar-filter option { background:#0e0b1a; }
        ...
        .ar-spinner { width:18px; height:18px; border-radius:50%; border:2px solid rgba(96,165,250,0.2); border-top-color:#60a5fa; animation:ar-spin 0.7s linear infinite; }
        ...
        .ar-error { background:rgba(239,68,68,0.08); border:0.5px solid rgba(239,68,68,0.2); border-radius:8px; padding:10px 14px; margin-bottom:14px; font-size:13px; color:#fca5a5; }
      `}</style>
      ...
      <div className="ar-page">
        ...
        <div className="ar-stats">
          ...
          <div className="ar-stat"
            ...
            <span className="ar-stat-value" style={{ color: '#60a5fa' }}>{invoices.length}</span>
          </div>
        </div>
