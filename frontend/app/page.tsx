      ...
      {chartType === 'bar' ? (
            ...
            return (
              <div key={i} style={{ flex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                <div style={{
                  ...
                  background: isActive ? color : `color-mix(in srgb, ${color} 33%, transparent)`,
                  borderRadius: '2px 2px 0 0',
                  transition: 'height 0.3s, background 0.2s',
                  boxShadow: isActive ? `0 0 6px color-mix(in srgb, ${color} 53%, transparent)` : 'none',
                }} />
              </div>
            );
          })}
        ...
        <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ height: h, display: 'block' }}>
          ...
          {(() => {
            ...
            return (
              <>
                <path d={d + ` L${w},${h} L0,${h} Z`} fill={`url(#lg-${color.replace(/[^a-z0-9]/gi,'')})`} />
                <path d={d} fill="none" stroke={`color-mix(in srgb, ${color} 60%, transparent)`} strokeWidth="1.5" strokeLinejoin="round" />
                {display.map((p, i) => {
                  const isActive = activeLabel ? p.label === activeLabel : i === display.length - 1;
                  return isActive
