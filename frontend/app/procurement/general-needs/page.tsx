...
function GNDetailDrawer({ gn, onClose, onAction }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 400, display: 'flex' }}>
      ...
      <div style={{ width: 720, background: 'var(--bg, #0a0712)', borderLeft: '0.5px solid rgba(251,146,60,0.15)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>

        {/* Header */}
        <div style={{ padding: '16px 20px', borderBottom: '0.5px solid var(--l06, rgba(255,255,255,0.06))', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, position: 'sticky', top: 0, background: 'var(--bg, #0a0712)', zIndex: 1 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-strong, #f1ede8)', ...MONO }}>{gn.gnNumber}</div>
            <div style={{ fontSize: 12, color: 'var(--w40, rgba(255,255,255,0.4))', marginTop: 2 }}>{gn.title}</div>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                {pendingLines.length > 0 && (
                  <button
                    onClick={toggleAll}
                    style={{ fontSize: 11, color: 'var(--w40, rgba(255,255,255,0.4))', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}
                  >
                ...
                <thead>
                  <tr>
                    {['', '#', 'Item', 'Qty', 'UOM', 'Required', 'Sugg. Supplier', 'Est. Cost', 'Status'].map(h => (
                      <th key={h} style={{ padding: '6px 8px', fontSize: 10, color: 'rgba(251,146,60,0.5)', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', textAlign: ['Qty', 'Est. Cost'].includes(h) ? 'right' : 'left', borderBottom: '0.5px solid var(--l06, rgba(255,255,255,0.06))', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {detail.lines?.map(line => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: '8px' }}>
                          {line.item ? (
                            <>
                              ...
                              <div style={{ color: 'var(--w55, rgba(255,255,255,0.55))', fontSize: 11, marginTop: 1 }}>{line.item.name}</div>
                            </>
                          ) : (
                            <div style={{ color: 'var(--w40, rgba(255,255,255,0.4))', fontSize: 11, fontStyle: 'italic' }}>{line.genericDescription || '—'}</div>
                          )}
                        </td>
                        ...
                        <td style={{ padding: '8px', fontSize: 11, color: 'var(--w50, rgba(255,255,255,0.5))' }}>{fmtDateShort(line.requiredDate)}</td>
                        <td style={{ padding: '8px', fontSize: 11 }}>
                          {line.suggestedSupplier ? (
                            <span style={{ color: 'var(--accent-violet, #a78bfa)' }}>{line.suggestedSupplier.code}</span>
                          ) : <span style={{ color: 'var(--w20, rgba(255,255,255,0.2))' }}>—</span>}
                        </td>
                        <td style={{ padding: '8px', textAlign: 'right', ...MONO, fontSize: 11, color: 'var(--w50, rgba(255,255,255,0.5))' }}>
                          {fmtAmt(line.estimatedUnitCost)}
                        </td>
                        <td style={{ padding: '8px' }}>
                          <span style={{ fontSize: 10, padding: '1px 7px', borderRadius: 10, background: line.status === 'converted' ? 'rgba(74,222,128,0.1)' : line.status === 'cancelled' ? 'rgba(248,113,113,0.1)' : 'rgba(251,191,36,0.1)', color: line.status === 'converted' ? 'var(--success, #4ade80)' : line.status === 'cancelled' ? 'var(--danger, #f87171)' : 'var(--warning, #fbbf24)', border: `0.5px solid ${line.status === 'converted' ? 'rgba(74,222,128,0.2)' : line.status === 'cancelled' ? 'rgba(248,113,113,0.2)' : 'rgba(251,191,36,0.2)'}` }}>
                            {line.status}
                          </span>
                        </td>
                      </tr>
              ...
              <div style={{ background: 'rgba(96,165,250,0.04)', border: '0.5px solid rgba(96,165,250,0.15)', borderRadius: 10, padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: convertOpen ? 12 : 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--accent-blue, #60a5fa)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    ...
                    {selectedLines.size > 0 && <span style={{ marginLeft: 6, color: 'var(--w40, rgba(255,255,255,0.4))' }}>({selectedLines.size} lines selected)</span>}
                  </span>
                  <button
                    type="button"
                    onClick={() => setConvertOpen(o => !o)}
                    style={{ fontSize: 11, color: convertOpen ? 'var(--danger, #f87171)' : 'var(--accent-blue, #60a5fa)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}
                  >
                ...
                {convertOpen && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {convertErr && <div style={{ background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.2)', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: 'var(--danger-subtle, #fca5a5)' }}>{convertErr}</div>}
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <label style={{ fontSize: 10, fontWeight: 500, color: 'var(--w40, rgba(255,255,255,0.4))', textTransform: 'uppercase', letterSpacing: '0.08em' }}>PR Title *</label>
                        <input
                          value={prTitle} onChange={e => setPrTitle(e.target.value)}
                          style={{ background: 'var(--surface, #0e0b1a)', border: '0.5px solid var(--w12, rgba(255,255,255,0.12))', borderRadius: 6, padding: '7px 10px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: 'var(--text-primary, #e2dfd8)', outline: 'none' }}
                        />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <label style={{ fontSize: 10, fontWeight: 500, color: 'var(--w40, rgba(255,255,255,0.4))', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Priority</label>
            ...
            {statusError && <div style={{ background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.2)', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: 'var(--danger-subtle, #fca5a5)' }}>{statusError}</div>}
  ...
  return (
    <>
      <style>{`
        ...
        .gn-box{background:var(--surface, #0e0b1a);border:0.5px solid rgba(251,146,60,0.2);border-radius:14px;width:100%;max-width:920px;margin:auto;position:relative;box-shadow:0 24px 60px rgba(0,0,0,0.7)}
        ...
        .gn-th{font-size:10px;color:rgba(251,146,60,0.5);text-transform:uppercase;letter-spacing:0.08em;padding:5px 6px;text-align:left;border-bottom:0.5px solid var(--l06, rgba(255,255,255,0.06));white-space:nowrap;font-weight:500}
        .gn-inp{background:var(--l04, rgba(255,255,255,0.04));border:0.5px solid var(--w10, rgba(255,255,255,0.1));border-radius:5px;padding:5px 7px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:var(--text-strong, #f1ede8);outline:none;width:100%}
        .gn-sel{background:var(--l04, rgba(255,255,255,0.04));border:0.5px solid var(--w10, rgba(255,255,255,0.1));border-radius:5px;padding:5px 7px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:var(--text-strong, #f1ede8);outline:none;width:100%}
        .gn-sel option{background:var(--surface, #0e0b1a)}
        .gn-section{font-size:10px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:var(--w25, rgba(255,255,255,0.25));padding:6px 0 4px;border-bottom:0.5px solid var(--l06, rgba(255,255,255,0.06));margin-top:4px;display:flex;align-items:center;justify-content:space-between}
        .gn-btn-add{background:var(--l04, rgba(255,255,255,0.04));border:0.5px solid var(--w10, rgba(255,255,255,0.1));border-radius:5px;padding:4px 10px;font-size:11px;color:var(--w50, rgba(255,255,255,0.5));cursor:pointer;font-family:'IBM Plex Sans',sans-serif}
        .gn-btn-rm{width:20px;height:20px;border-radius:4px;background:rgba(239,68,68,0.1);border:0.5px solid rgba(239,68,68,0.2);color:var(--danger, #f87171);cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
      `}</style>
      <div className="gn-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="gn-box">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px 12px', borderBottom: '0.5px solid var(--l06, rgba(255,255,255,0.06))', position: 'sticky', top: 0, background: 'var(--surface, #0e0b1a)', zIndex: 1, borderRadius: '14px 14px 0 0' }}>
            <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-strong, #f1ede8)' }}>New General Need</span>
            <button onClick={onClose} style={{ width: 24, height: 24, borderRadius: 6, background: 'var(--l06, rgba(255,255,255,0.06))', border: 'none', cursor: 'pointer', color: 'var(--w45, rgba(255,255,255,0.45))', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          </div>
  ...
  const columns = useMemo<ERPColumn<GN>[]>(() => [
    {
      ...
      value: r => r.gnNumber,
      render: r => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: 'var(--accent-strong, #fb923c)', fontWeight: 500 }}>{r.gnNumber}</span>,
    },
    {
    ...
    },
    {
      ...
      value: r => r.source,
      render: r => <span style={{ fontSize: 11, color: 'var(--w40, rgba(255,255,255,0.4))' }}>{r.source === 'mrp_explode' ? 'MRP' : 'Manual'}</span>,
    },
    {
    ...
    },
    {
      ...
      render: r => {
        ...
        return <span style={{ fontSize: 12, color: count > 0 ? 'var(--accent-blue, #60a5fa)' : 'var(--w25, rgba(255,255,255,0.25))' }}>{count}</span>;
      },
    },
    {
      ...
      <div className="gn-page">
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 10, flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          <div onClick={() => setActiveStatus(null)}
            ...
            <span style={{ fontSize: 22, fontWeight: 500, color: 'var(--accent-strong, #fb923c)', fontFamily: "'IBM Plex Mono',monospace" }}>{gns.length}</span>
          </div>
        </div>
