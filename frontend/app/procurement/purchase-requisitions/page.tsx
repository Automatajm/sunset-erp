...
const STATUS_CFG: Record<PRStatus, { color: string; bg: string; border: string; label: string }> = {
  ...
  submitted:   { color: '#60a5fa', bg: 'rgba(96,165,250,0.1)',  border: 'rgba(96,165,250,0.2)',  label: 'Submitted' },
...
function PRDetailDrawer({ pr, onClose, onAction, suppliers }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 400, display: 'flex' }}>
      ...
      <div style={{ width: 740, background: '#0a0712', borderLeft: '0.5px solid rgba(251,146,60,0.15)', display: 'flex', flexDirection: 'column' }}>

        {/* Header */}
        <div style={{ padding: '16px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, position: 'sticky', top: 0, background: '#0a0712', zIndex: 1 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, color: '#f1ede8', ...MONO }}>{pr.prNumber}</div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {detail.status === 'rejected' && detail.rejectionReason && (
              <div style={{ background: 'rgba(248,113,113,0.06)', border: '0.5px solid rgba(248,113,113,0.2)', borderRadius: 8, padding: '10px 14px' }}>
                <div style={{ fontSize: 10, color: '#f87171', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Rejection Reason</div>
            ...
            {detail.rfqs && detail.rfqs.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {detail.rfqs.map(rfq => (
                    <span key={rfq.id} style={{ ...MONO, fontSize: 11, padding: '3px 10px', borderRadius: 20, background: 'rgba(96,165,250,0.08)', border: '0.5px solid rgba(96,165,250,0.2)', color: '#60a5fa' }}>
                  ...
                  {detail.lines?.map(line => {
                    ...
                    return (
                      <tr key={line.id}
                        ...
                        <td style={{ padding: '8px' }}>
                          <span style={{ fontSize: 10, padding: '1px 7px', borderRadius: 10, background: line.itemStatus === 'catalog' ? 'rgba(74,222,128,0.1)' : 'rgba(251,191,36,0.1)', color: line.itemStatus === 'catalog' ? '#4ade80' : '#fbbf24', border: `0.5px solid ${line.itemStatus === 'catalog' ? 'rgba(74,222,128,0.2)' : 'rgba(251,191,36,0.2)'}` }}>
                            {line.itemStatus}
                          </span>
                        </td>
                      </tr>
            ...
            {canConvert && (
              <div style={{ background: 'rgba(96,165,250,0.04)', border: '0.5px solid rgba(96,165,250,0.15)', borderRadius: 10, padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: rfqOpen ? 12 : 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 500, color: '#60a5fa', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    Convert to RFQ
                    {selectedLines.size > 0 && <span style={{ marginLeft: 6, color: 'rgba(255,255,255,0.4)' }}>({selectedLines.size} lines)</span>}
                  </span>
                  <button type="button" onClick={() => setRfqOpen(o => !o)} style={{ fontSize: 11, color: rfqOpen ? '#f87171' : '#60a5fa', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif" }}>
                ...
                {rfqOpen && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {rfqError && <div style={{ background: 'rgba(239,68,68,0.1)', border: '0.5px solid rgba(239,68,68,0.2)', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: '#fca5a5' }}>{rfqError}</div>}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        <input value={rfqTitle} onChange={e => setRfqTitle(e.target.value)}
                          style={{ background: '#0e0b1a', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 6, padding: '7px 10px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: '#e2dfd8', outline: 'none' }} />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        <input type="date" value={rfqDeadline} onChange={e => setRfqDeadline(e.target.value)}
                          style={{ background: '#0e0b1a', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: 6, padding: '7px 10px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: '#e2dfd8', outline: 'none', colorScheme: 'dark' as any }} />
                      </div>
                    </div>
                        ...
                        {suppliers.map(s => {
                          ...
                          return (
                            <span key={s.id} onClick={() => toggleSupplier(s.id)}
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 20, fontSize: 11, cursor: 'pointer', background: sel ? 'rgba(167,139,250,0.1)' : 'rgba(255,255,255,0.03)', border: `0.5px solid ${sel ? 'rgba(167,139,250,0.35)' : 'rgba(255,255,255,0.1)'}`, color: sel ? '#a78bfa' : 'rgba(255,255,255,0.4)', transition: 'all 0.1s', fontFamily: "'IBM Plex Sans',sans-serif" }}>
            ...
            {rejectOpen && (
              <div style={{ background: 'rgba(248,113,113,0.05)', border: '0.5px solid rgba(248,113,113,0.2)', borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 500, color: '#f87171', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Rejection Reason *</div>
                <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)} rows={3}
                  placeholder="Explain why this PR is being rejected…"
                  style={{ background: '#0e0b1a', border: '0.5px solid rgba(248,113,113,0.2)', borderRadius: 6, padding: '8px 10px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: '#e2dfd8', outline: 'none', resize: 'vertical' }} />
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                  ...
                  <button onClick={() => handleStatus('rejected', rejectReason)} disabled={!rejectReason.trim() || actionBusy}
                    style={{ padding: '6px 14px', borderRadius: 6, fontSize: 12, cursor: 'pointer', background: 'rgba(248,113,113,0.15)', border: '0.5px solid rgba(248,113,113,0.3)', color: '#f87171', fontFamily: "'IBM Plex Sans',sans-serif", opacity: !rejectReason.trim() ? 0.5 : 1 }}>
                    Confirm Rejection
                  </button>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, paddingTop: 8, borderTop: '0.5px solid rgba(255,255,255,0.06)', flexWrap: 'wrap' }}>
              ...
              {canReject && !rejectOpen && (
                <button onClick={() => setRejectOpen(true)} disabled={actionBusy}
                  style={{ padding: '7px 16px', borderRadius: 7, fontSize: 12, cursor: 'pointer', background: 'rgba(248,113,113,0.08)', border: '0.5px solid rgba(248,113,113,0.2)', color: '#f87171', fontFamily: "'IBM Plex Sans',sans-serif", opacity: actionBusy ? 0.5 : 1 }}>
                  ✕ Reject
                </button>
              )}
              {canCancel && (
...
function CreatePRModal({ open, onClose, onSaved, items, warehouses }: {
  ...
  const PRIORITY_COLORS: Record<string, string> = { normal: 'rgba(255,255,255,0.4)', urgent: '#fb923c', critical: '#f87171' };
  ...
  return (
    <>
      <style>{`
        ...
        .pr-box{background:#0e0b1a;border:0.5px solid rgba(251,146,60,0.2);border-radius:14px;width:100%;max-width:940px;margin:auto;position:relative;box-shadow:0 24px 60px rgba(0,0,0,0.7)}
        ...
        .pr-inp{background:rgba(255,255,255,0.04);border:0.5px solid rgba(255,255,255,0.1);border-radius:5px;padding:5px 7px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:#f1ede8;outline:none;width:100%}
        .pr-sel{background:rgba(255,255,255,0.04);border:0.5px solid rgba(255,255,255,0.1);border-radius:5px;padding:5px 7px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:#f1ede8;outline:none;width:100%}
        .pr-sel option{background:#0e0b1a}
        ...
        .pr-btn-rm{width:20px;height:20px;border-radius:4px;background:rgba(239,68,68,0.1);border:0.5px solid rgba(239,68,68,0.2);color:#f87171;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
      `}</style>
      <div className="pr-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="pr-box">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px 12px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, background: '#0e0b1a', zIndex: 1, borderRadius: '14px 14px 0 0' }}>
            <span style={{ fontSize: 14, fontWeight: 500, color: '#f1ede8' }}>New Purchase Requisition</span>
            <button onClick={onClose} style={{ width: 24, height: 24, borderRadius: 6, background: 'rgba(255,255,255,0.06)', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.45)', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          </div>
            ...
            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr', gap: 10 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  ...
                  <select value={header.priority} onChange={setH('priority')} style={{ ...INP, cursor: 'pointer', color: PRIORITY_COLORS[header.priority] ?? '#f1ede8' }}>
  ...
  const columns = useMemo<ERPColumn<PR>[]>(() => [
    {
      ...
      value: r => r.prNumber,
      render: r => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: '#fb923c', fontWeight: 500 }}>{r.prNumber}</span>,
    },
    {
    ...
    },
    {
      ...
      render: r => {
        ...
        return <span style={{ fontSize: 12, color: isLate ? '#f87171' : 'rgba(255,255,255,0.5)' }}>{fmtDateShort(r.requiredDate)}</span>;
      },
    },
    {
    ...
    },
    {
      ...
      render: r => {
        ...
        return <span style={{ fontSize: 12, color: count > 0 ? '#60a5fa' : 'rgba(255,255,255,0.25)' }}>{count}</span>;
      },
    },
    {
      key: 'estimatedAmount', header: 'Est. Amount', width: 120, align: 'right', sortable: true,
      value: r => Number(r.estimatedAmount ?? 0),
      render: r => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: r.estimatedAmount ? '#e2dfd8' : 'rgba(255,255,255,0.2)' }}>{fmtAmt(r.estimatedAmount)}</span>,
    },
    {
      ...
      <div className="pr-page">
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 10, flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          <div onClick={() => setActiveStatus(null)}
            ...
            <span style={{ fontSize: 22, fontWeight: 500, color: '#fb923c', fontFamily: "'IBM Plex Mono',monospace" }}>{prs.length}</span>
          </div>
        </div>
