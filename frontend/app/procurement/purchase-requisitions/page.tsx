...
const PRIORITY_CFG: Record<PRPriority, { color: string; label: string }> = {
  normal:   { color: 'var(--w35, rgba(255,255,255,0.35))', label: 'Normal' },
      ...
      <div style={{ width: 740, background: 'var(--bg, #0a0712)', borderLeft: '0.5px solid rgba(251,146,60,0.15)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '0.5px solid var(--l06, rgba(255,255,255,0.06))', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, position: 'sticky', top: 0, background: 'var(--bg, #0a0712)', zIndex: 1 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--w40, rgba(255,255,255,0.4))', marginTop: 2 }}>{pr.title}</div>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {detail.status === 'rejected' && detail.rejectionReason && (
              <div style={{ background: 'rgba(248,113,113,0.06)', border: '0.5px solid rgba(248,113,113,0.2)', borderRadius: 8, padding: '10px 14px' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--w60, rgba(255,255,255,0.6))' }}>{detail.rejectionReason}</div>
              </div>
            )}

            {/* Justification */}
            {detail.justification && (
              <div style={{ background: 'var(--l02, rgba(255,255,255,0.02))', borderRadius: 8, padding: '10px 14px' }}>
                <div style={{ fontSize: 10, color: 'var(--w30, rgba(255,255,255,0.3))', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Justification</div>
                <div style={{ fontSize: 12, color: 'var(--w60, rgba(255,255,255,0.6))', lineHeight: 1.5 }}>{detail.justification}</div>
              </div>
            )}
                  ...
                  {detail.lines?.map(line => {
                    ...
                    return (
                      <tr key={line.id}
                        ...
                        style={{ borderBottom: '0.5px solid var(--l04, rgba(255,255,255,0.04))', cursor: canConvert ? 'pointer' : 'default', background: isSelected ? 'rgba(96,165,250,0.06)' : 'transparent', transition: 'background 0.1s' }}>
                        ...
                        <td style={{ padding: '8px', fontSize: 11, color: 'var(--w45, rgba(255,255,255,0.45))' }}>{line.warehouse?.code ?? '—'}</td>
                        <td style={{ padding: '8px', textAlign: 'right', ...MONO, fontSize: 11, color: 'var(--w50, rgba(255,255,255,0.5))' }}>{fmtAmt(line.unitEstimate)}</td>
                        <td style={{ padding: '8px' }}>
              ...
              <div style={{ background: 'rgba(96,165,250,0.04)', border: '0.5px solid rgba(96,165,250,0.15)', borderRadius: 10, padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: rfqOpen ? 12 : 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--accent-blue, #60a5fa)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    Convert to RFQ
                    {selectedLines.size > 0 && <span style={{ marginLeft: 6, color: 'var(--w40, rgba(255,255,255,0.4))' }}>({selectedLines.size} lines)</span>}
                  </span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <label style={{ fontSize: 10, fontWeight: 500, color: 'var(--w40, rgba(255,255,255,0.4))', textTransform: 'uppercase', letterSpacing: '0.08em' }}>RFQ Title *</label>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <label style={{ fontSize: 10, fontWeight: 500, color: 'var(--w40, rgba(255,255,255,0.4))', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Response Deadline</label>
                        <input type="date" value={rfqDeadline} onChange={e => setRfqDeadline(e.target.value)}
                          style={{ background: 'var(--surface, #0e0b1a)', border: '0.5px solid var(--w12, rgba(255,255,255,0.12))', borderRadius: 6, padding: '7px 10px', fontSize: 12, fontFamily: "'IBM Plex Sans',sans-serif", color: 'var(--text-primary, #e2dfd8)', outline: 'none', colorScheme: 'dark' as any }} />
                      </div>
                    </div>

                    <div>
                      <label style={{ fontSize: 10, fontWeight: 500, color: 'var(--w40, rgba(255,255,255,0.4))', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>
                        ...
                        {suppliers.map(s => {
                          ...
                          return (
                            <span key={s.id} onClick={() => toggleSupplier(s.id)}
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 20, fontSize: 11, cursor: 'pointer', background: sel ? 'rgba(167,139,250,0.1)' : 'var(--l03, rgba(255,255,255,0.03))', border: `0.5px solid ${sel ? 'rgba(167,139,250,0.35)' : 'var(--w10, rgba(255,255,255,0.1))'}`, color: sel ? 'var(--accent-violet, #a78bfa)' : 'var(--w40, rgba(255,255,255,0.4))', transition: 'all 0.1s', fontFamily: "'IBM Plex Sans',sans-serif" }}>
            ...
            {rejectOpen && (
              <div style={{ background: 'rgba(248,113,113,0.05)', border: '0.5px solid rgba(248,113,113,0.2)', borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                  <button onClick={() => { setRejectOpen(false); setRejectReason(''); }} style={{ padding: '6px 14px', borderRadius: 6, fontSize: 12, cursor: 'pointer', background: 'var(--l05, rgba(255,255,255,0.05))', border: '0.5px solid var(--w10, rgba(255,255,255,0.1))', color: 'var(--w50, rgba(255,255,255,0.5))', fontFamily: "'IBM Plex Sans',sans-serif" }}>Cancel</button>
...
function CreatePRModal({ open, onClose, onSaved, items, warehouses }: {
  ...
  const PRIORITY_COLORS: Record<string, string> = { normal: 'var(--w40, rgba(255,255,255,0.4))', urgent: 'var(--accent-strong, #fb923c)', critical: 'var(--danger, #f87171)' };
  ...
  return (
    <>
      <style>{`
        ...
        .pr-th{font-size:10px;color:rgba(251,146,60,0.5);text-transform:uppercase;letter-spacing:0.08em;padding:5px 6px;text-align:left;border-bottom:0.5px solid var(--l06, rgba(255,255,255,0.06));white-space:nowrap;font-weight:500}
        .pr-inp{background:var(--l04, rgba(255,255,255,0.04));border:0.5px solid var(--w10, rgba(255,255,255,0.1));border-radius:5px;padding:5px 7px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:var(--text-strong, #f1ede8);outline:none;width:100%}
        .pr-sel{background:var(--l04, rgba(255,255,255,0.04));border:0.5px solid var(--w10, rgba(255,255,255,0.1));border-radius:5px;padding:5px 7px;font-size:12px;font-family:'IBM Plex Sans',sans-serif;color:var(--text-strong, #f1ede8);outline:none;width:100%}
        .pr-sel option{background:var(--surface, #0e0b1a)}
        .pr-section{font-size:10px;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;color:var(--w25, rgba(255,255,255,0.25));padding:6px 0 4px;border-bottom:0.5px solid var(--l06, rgba(255,255,255,0.06));margin-top:4px;display:flex;align-items:center;justify-content:space-between}
        .pr-btn-add{background:var(--l04, rgba(255,255,255,0.04));border:0.5px solid var(--w10, rgba(255,255,255,0.1));border-radius:5px;padding:4px 10px;font-size:11px;color:var(--w50, rgba(255,255,255,0.5));cursor:pointer;font-family:'IBM Plex Sans',sans-serif}
  ...
  const columns = useMemo<ERPColumn<PR>[]>(() => [
    ...
    },
    {
      ...
      render: r => {
        ...
        return <span style={{ fontSize: 12, color: isLate ? 'var(--danger, #f87171)' : 'var(--w50, rgba(255,255,255,0.5))' }}>{fmtDateShort(r.requiredDate)}</span>;
      },
    },
    {
      key: 'source', header: 'Source', width: 110, sortable: true,
      value: r => r.source,
      render: r => <span style={{ fontSize: 11, color: 'var(--w40, rgba(255,255,255,0.4))' }}>{r.source.replace(/_/g, ' ')}</span>,
    },
    {
    ...
    },
    {
      ...
      render: r => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: r.estimatedAmount ? 'var(--text-primary, #e2dfd8)' : 'var(--w20, rgba(255,255,255,0.2))' }}>{fmtAmt(r.estimatedAmount)}</span>,
    },
    {
