...
function ExpiryBadge({ status, days }: { status?: string; days?: number | null }) {
  ...
  const cfg: Record<string, { color: string; bg: string; label: string }> = {
    ...
    critical:     { color: 'var(--accent-mid, #f97316)', bg: 'rgba(249,115,22,0.15)', label: `${days}d`             },
...
function Btn({ children, onClick, variant = 'ghost' }: { children: React.ReactNode; onClick: () => void; variant?: 'primary' | 'ghost' }) {
  const styles: Record<string, React.CSSProperties> = {
    primary: { background: 'linear-gradient(135deg,var(--accent-pressed, #c2410c),var(--accent-mid, #f97316))', border: 'none', color: 'white', fontWeight: 600 },
    ghost:   { background: 'rgba(251,146,60,0.08)', border: '0.5px solid rgba(251,146,60,0.3)', color: 'var(--accent-strong, #fb923c)' },
  };
  return (
...
function InlineEdit({ value, onSave, type = 'text', suffix = '', min, step, width = 80, options }: {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    color: 'var(--warning, #fbbf24)', outline: 'none',
          ...
          {si.isBlocked ? (
            <>
              ...
              <button onClick={() => toggle(false)} disabled={saving}
                style={{ width: '100%', padding: '6px', borderRadius: 6, fontSize: 11, cursor: 'pointer', background: 'rgba(74,222,128,0.1)', border: '0.5px solid rgba(74,222,128,0.3)', color: 'var(--success, #4ade80)', fontFamily: "'IBM Plex Sans',sans-serif" }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input type="number" min="0" step="0.01" value={price} onChange={e => setPrice(e.target.value)}
              style={{ ...INP, ...MONO, color: 'var(--warning, #fbbf24)', border: '0.5px solid rgba(251,146,60,0.3)' }} />
          </div>
          ...
          {err && <div style={{ fontSize: 11, color: 'var(--danger, #f87171)', background: 'rgba(248,113,113,0.08)', borderRadius: 6, padding: '6px 10px' }}>{err}</div>}
...
function PriceHistoryPanel({ siId, onClose }: { siId: string; onClose: () => void }) {
  ...
  const srcColor: Record<string, string> = { rfq: 'var(--accent-violet, #a78bfa)', manual: 'var(--warning, #fbbf24)', import: 'var(--success, #4ade80)', grn: '#38bdf8' };
...
function IncotermInfoModal({ onClose }: { onClose: () => void }) {
  const groups = [
    { label: 'Any Mode of Transport', keys: ['EXW','FCA','CPT','CIP','DAP','DPU','DDP'], color: 'var(--accent-blue, #60a5fa)' },
  ...
  return (
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 600, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#0c0a18', border: '0.5px solid rgba(251,146,60,0.25)', borderRadius: 14, width: 500, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.7)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {err && <div style={{ fontSize: 11, color: 'var(--danger, #f87171)', background: 'rgba(248,113,113,0.08)', border: '0.5px solid rgba(248,113,113,0.2)', borderRadius: 6, padding: '8px 12px' }}>{err}</div>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <div>
              ...
              <input type="number" min="0" step="0.01" value={lastPrice} onChange={e => setLastPrice(e.target.value)} placeholder="0.00" style={{ ...INP, ...MONO, color: 'var(--warning, #fbbf24)' }} />
            </div>
            <div>
              <label style={LBL}>Valid From</label>
              <input type="date" value={validFrom} onChange={e => setValidFrom(e.target.value)} style={INP} />
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                ...
                <button type="button" onClick={() => setShowIncotermInfo(true)}
                  style={{ width: 16, height: 16, borderRadius: '50%', background: 'rgba(96,165,250,0.15)', border: '0.5px solid rgba(96,165,250,0.3)', color: 'var(--accent-blue, #60a5fa)', cursor: 'pointer', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, fontFamily: 'serif' }}>
                  ?
                </button>
              </div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        {selectedId && (
          <button onClick={() => setAddModal(true)}
            style={{ marginLeft: 'auto', padding: '7px 14px', borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: 'pointer', background: 'linear-gradient(135deg,var(--accent-pressed, #c2410c),var(--accent-mid, #f97316))', border: 'none', color: 'white', fontFamily: "'IBM Plex Sans',sans-serif", whiteSpace: 'nowrap', boxShadow: '0 2px 8px rgba(234,88,12,0.3)' }}>
            + Add Item
          </button>
        )}
      </div>
...
const ALERT_FILTERS: ERPFilter<any>[] = [
  {
    ...
    options: [
      { value: 'expired',       label: 'Expired',  color: '#ef4444', bg: 'rgba(239,68,68,0.1)',    border: 'rgba(239,68,68,0.3)'   },
      { value: 'expires_today', label: 'Today',    color: 'var(--accent-mid, #f97316)', bg: 'rgba(249,115,22,0.1)',   border: 'rgba(249,115,22,0.3)'  },
      { value: 'critical',      label: 'Critical', color: 'var(--warning, #fbbf24)', bg: 'rgba(251,191,36,0.1)',   border: 'rgba(251,191,36,0.3)'  },
      { value: 'warning',       label: 'Warning',  color: 'var(--accent-violet, #a78bfa)', bg: 'rgba(167,139,250,0.1)',  border: 'rgba(167,139,250,0.3)' },
    ],
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 20, marginLeft: 'auto' }}>
          {[
            ...
            { label: 'Today',    count: counts.expires_today, color: 'var(--accent-mid, #f97316)' },
            { label: 'Critical', count: counts.critical,      color: 'var(--warning, #fbbf24)' },
            { label: 'Warning',  count: counts.warning,       color: 'var(--accent-violet, #a78bfa)' },
...
export default function SupplierItemsPage() {
  ...
  return (
    <ERPShell>
      ...
      <div style={{ padding: '24px 28px', minHeight: '100vh', background: '#08061a', fontFamily: "'IBM Plex Sans',sans-serif", color: 'var(--text-strong, #f1ede8)' }}>
