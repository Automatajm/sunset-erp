...
function UserModal({ user, roles, onClose, onSaved }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#0a0712', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 14, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '16px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: '#e2dfd8' }}>{isEdit ? 'Edit User' : 'New User'}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
              {roles.map(role => (
                <label key={role.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 8, cursor: 'pointer', background: form.roleIds.includes(role.id) ? 'rgba(251,146,60,0.08)' : 'rgba(255,255,255,0.02)', border: `0.5px solid ${form.roleIds.includes(role.id) ? 'rgba(251,146,60,0.25)' : 'rgba(255,255,255,0.07)'}` }}>
                  <input type="checkbox" checked={form.roleIds.includes(role.id)} onChange={() => toggleRole(role.id)} style={{ accentColor: '#fb923c' }} />
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: form.roleIds.includes(role.id) ? '#fb923c' : '#e2dfd8' }}>{role.name}</div>
...
function ResetPasswordModal({ user, onClose, onSaved }: { user: User; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#0a0712', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 14, width: '100%', maxWidth: 380, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '16px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: '#e2dfd8' }}>Reset Password</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 18 }}>x</button>
        </div>
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Setting new password for <strong style={{ color: '#fb923c' }}>{user.fullName}</strong></div>
          <div>
    ...
    <div style={{ minHeight: '100vh', background: '#06040f', color: '#e2dfd8', fontFamily: "'IBM Plex Sans',sans-serif" }}>
      ...
      <div style={{ padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {[
            { label: 'Total Users', value: users.length, color: '#e2dfd8' },
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden' }}>
            ...
            {filtered.length === 0 ? (
              ...
              <div key={user.id} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 2fr 1fr', gap: 0, padding: '12px 16px', borderBottom: i < filtered.length - 1 ? '0.5px solid rgba(255,255,255,0.04)' : 'none', alignItems: 'center' }}>
                {/* Name */}
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: user.isActive ? '#e2dfd8' : 'rgba(255,255,255,0.35)' }}>{user.fullName}</div>
                ...
                {/* Status */}
                <div>
                  <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, fontWeight: 600, background: user.isActive ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)', color: user.isActive ? '#4ade80' : '#f87171', border: `0.5px solid ${user.isActive ? 'rgba(74,222,128,0.2)' : 'rgba(248,113,113,0.2)'}` }}>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {user.roles.length === 0
                    ...
                    : user.roles.map(r => (
                        <span key={r.id} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, background: 'rgba(251,146,60,0.1)', color: '#fb923c', border: '0.5px solid rgba(251,146,60,0.2)' }}>{r.name}</span>
                      ))
                  }
                </div>
                {/* Actions */}
