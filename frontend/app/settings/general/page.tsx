...
function WarningModal({ payload, onCancel }: { payload: WarnPayload; onCancel: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#0e0b1a', border: '0.5px solid rgba(251,191,36,0.3)', borderRadius: 14, width: '100%', maxWidth: 500, boxShadow: '0 24px 60px rgba(0,0,0,0.8)' }}>
        ...
        <div style={{ padding: '16px 20px 12px', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 500, color: '#fbbf24' }}>Confirm System UOM Changes</div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, overflow: 'hidden' }}>
            {payload.changes.map((c, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 14px', borderBottom: i < payload.changes.length - 1 ? '0.5px solid rgba(255,255,255,0.05)' : 'none' }}>
                ...
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 13, fontWeight: 500, color: '#4ade80' }}>{c.fromCode}</span>
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.25)' }}>→</span>
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 13, fontWeight: 500, color: '#f87171' }}>{c.toCode}</span>
              </div>
            ))}
          </div>
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Settings', 'General']} title="General Settings">
      <style>{`
        ...
        .gs-btn-save{background:linear-gradient(135deg,#c2410c,#ea580c,#f97316);border:none;border-radius:7px;padding:8px 22px;font-size:13px;font-weight:500;font-family:'IBM Plex Sans',sans-serif;color:white;cursor:pointer;box-shadow:0 3px 12px rgba(234,88,12,0.35)}
          ...
          <div className="gs-layout">
            ...
            <div className="gs-col">
                ...
                <div className="gs-body">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ background: 'rgba(251,146,60,0.08)', border: '0.5px solid rgba(251,146,60,0.2)', borderRadius: 7, padding: '5px 12px' }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 500, fontFamily: "'IBM Plex Mono',monospace", color: '#fb923c' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {savedSystemUoms.map(({ label, type, uom }) => {
                      const color = UOM_TYPE_COLOR[type] ?? '#e2dfd8';
                      return (
                        <div key={label} style={{ background: `${color}0d`, border: `0.5px solid ${color}28`, borderRadius: 7, padding: '5px 10px' }}>
                          <span style={{ fontSize: 9, color: `${color}65`, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block' }}>{label}</span>
                    ...
                    {(['metric', 'imperial', 'custom'] as UomSystem[]).map(sys => {
                      ...
                      return (
                        <div key={sys} onClick={() => handleSystemChange(sys)}
                          style={{ padding: '10px 14px', borderRadius: 8, cursor: 'pointer', fontFamily: "'IBM Plex Sans',sans-serif", fontSize: 13, fontWeight: 500, border: `0.5px solid ${active ? 'rgba(251,146,60,0.4)' : 'rgba(255,255,255,0.07)'}`, transition: 'all 0.15s', color: active ? '#fb923c' : 'rgba(255,255,255,0.35)', background: active ? 'rgba(251,146,60,0.07)' : 'rgba(255,255,255,0.02)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ background: 'rgba(96,165,250,0.04)', border: '0.5px solid rgba(96,165,250,0.1)', borderRadius: 10, padding: '10px 14px', fontSize: 11, color: 'rgba(255,255,255,0.28)', lineHeight: 1.7 }}>
                <strong style={{ color: '#60a5fa' }}>How it works:</strong> Formulators, buyers and warehouse staff work in any unit.
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 10 }}>
                {hasAnyDirty && (
                  <span style={{ fontSize: 11, color: '#fbbf24', display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#fbbf24', boxShadow: '0 0 4px rgba(251,191,36,0.6)', display: 'inline-block' }} />
                    Unsaved changes
                  </span>
                )}
            ...
            <div className="gs-col">
              <div className="gs-section" style={{ flex: 1 }}>
                ...
                <div className="gs-body">
                  {UOM_ROWS.map(row => {
                    const color    = UOM_TYPE_COLOR[row.type] ?? '#e2dfd8';
                          ...
                          {dirty && selUom && selUom.id !== savedId && (
                            <div style={{ marginTop: 5, background: 'rgba(251,191,36,0.07)', borderRadius: 6, padding: '5px 8px', border: '0.5px solid rgba(251,191,36,0.2)', display: 'flex', flexDirection: 'column', gap: 2 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                                ...
                                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: '#fbbf24', fontWeight: 500 }}>{selUom.code}</span>
