...
function PostingBadge({ allowed }: { allowed: boolean }) {
  return (
    <span style={{
      ...
      fontSize: 10, color: allowed ? 'var(--success, #4ade80)' : 'rgba(255,255,255,0.25)',
    }}>
      <span style={{ width: 5, height: 5, borderRadius: '50%', background: allowed ? 'var(--success, #4ade80)' : 'rgba(255,255,255,0.2)', flexShrink: 0 }} />
  ...
  return (
    <>
      <style>{`
        ...
        .am-box { background:var(--surface, #0e0b1a); border:0.5px solid rgba(251,146,60,0.2); border-radius:14px; width:100%; max-width:540px; max-height:92vh; overflow-y:auto; position:relative; box-shadow:0 24px 60px rgba(0,0,0,0.7); }
        ...
        .am-hdr { display:flex; align-items:center; justify-content:space-between; padding:16px 20px 12px; border-bottom:0.5px solid rgba(255,255,255,0.06); position:sticky; top:0; background:var(--surface, #0e0b1a); z-index:1; }
        .am-title { font-size:14px; font-weight:500; color:var(--text-strong, #f1ede8); font-family:'IBM Plex Sans',sans-serif; }
        ...
        .am-input, .am-select { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.1); border-radius:7px; padding:9px 12px; font-size:13px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-strong, #f1ede8); outline:none; width:100%; transition:border-color 0.2s, box-shadow 0.2s; }
        ...
        .am-select option { background:var(--surface, #0e0b1a); color:var(--text-strong, #f1ede8); }
        ...
        .am-error { background:rgba(239,68,68,0.1); border:0.5px solid rgba(239,68,68,0.25); border-radius:7px; padding:8px 12px; font-size:12px; color:var(--danger-subtle, #fca5a5); }
        ...
        .am-btn-save { background:linear-gradient(135deg,var(--accent-pressed, #c2410c),var(--accent, #ea580c),var(--accent-mid, #f97316)); border:none; border-radius:7px; padding:8px 20px; font-size:13px; font-weight:500; font-family:'IBM Plex Sans',sans-serif; color:white; cursor:pointer; box-shadow:0 3px 12px rgba(234,88,12,0.35); transition:opacity 0.2s; }
              ...
              <div className="am-toggles">
                {([
                  ...
                  <label key={key} className="am-toggle-label"
                    style={{ color: form[key] ? 'var(--text-primary, #e2dfd8)' : 'rgba(255,255,255,0.4)' }}>
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'var(--surface, #0e0b1a)', border:'0.5px solid rgba(239,68,68,0.25)', borderRadius:14, width:'100%', maxWidth:420, padding:'24px 24px 20px', boxShadow:'0 24px 60px rgba(0,0,0,0.7)' }}>
        <div style={{ fontSize:14, fontWeight:500, color:'var(--text-strong, #f1ede8)', marginBottom:10 }}>Delete account?</div>
        <div style={{ fontSize:13, color:'rgba(255,255,255,0.5)', marginBottom:20, lineHeight:1.5 }}>
          <strong style={{ color:'var(--text-strong, #f1ede8)' }}>{account.accountNumber} — {account.name}</strong>{' '}
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Financial', 'Chart of Accounts']} title="Chart of Accounts">
      <style>{`
        ...
        .coa-search { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-primary, #e2dfd8); outline:none; width:260px; transition:border-color 0.2s; }
        ...
        .coa-filter { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-primary, #e2dfd8); outline:none; }
        .coa-filter option { background:var(--surface, #0e0b1a); color:var(--text-strong, #f1ede8); }
        .coa-btn-new { display:flex; align-items:center; gap:6px; margin-left:auto; background:linear-gradient(135deg,var(--accent-pressed, #c2410c),var(--accent, #ea580c),var(--accent-mid, #f97316)); border:none; border-radius:7px; padding:7px 14px; font-size:12px; font-weight:500; font-family:'IBM Plex Sans',sans-serif; color:white; cursor:pointer; box-shadow:0 3px 12px rgba(234,88,12,0.3); transition:opacity 0.15s, transform 0.15s; flex-shrink:0; }
        ...
        .coa-num  { font-family:'IBM Plex Mono',monospace; font-size:13px; color:var(--accent-strong, #fb923c); font-weight:500; }
        .coa-name { color:var(--text-primary, #e2dfd8); font-weight:500; }
        ...
        .coa-btn-del { background:rgba(239,68,68,0.08); color:var(--danger, #f87171); border-color:rgba(239,68,68,0.2); }
        ...
        .coa-spinner { width:18px; height:18px; border-radius:50%; border:2px solid rgba(251,146,60,0.2); border-top-color:var(--accent-strong, #fb923c); animation:coa-spin 0.7s linear infinite; flex-shrink:0; }
        ...
        .coa-error { background:rgba(239,68,68,0.08); border:0.5px solid rgba(239,68,68,0.2); border-radius:8px; padding:10px 14px; margin-bottom:14px; font-size:13px; color:var(--danger-subtle, #fca5a5); }
      `}</style>
