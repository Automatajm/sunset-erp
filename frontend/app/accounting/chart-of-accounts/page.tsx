...
const ACCOUNT_TYPES: {
  ...
  { value: 'asset',     label: 'Asset',     color: 'var(--accent-blue)', bg: 'rgba(96,165,250,0.1)',  border: 'rgba(96,165,250,0.2)' },
  { value: 'liability', label: 'Liability', color: 'var(--danger)', bg: 'rgba(248,113,113,0.1)', border: 'rgba(248,113,113,0.2)' },
  { value: 'equity',    label: 'Equity',    color: 'var(--accent-violet)', bg: 'rgba(167,139,250,0.1)', border: 'rgba(167,139,250,0.2)' },
  { value: 'revenue',   label: 'Revenue',   color: 'var(--success)', bg: 'rgba(74,222,128,0.1)',  border: 'rgba(74,222,128,0.2)' },
  { value: 'cost',      label: 'Cost',      color: 'var(--warning)', bg: 'rgba(251,191,36,0.1)',  border: 'rgba(251,191,36,0.2)'  },
  { value: 'expense',   label: 'Expense',   color: 'var(--accent-strong)', bg: 'rgba(251,146,60,0.1)',  border: 'rgba(251,146,60,0.18)' },
];
...
function PostingBadge({ allowed }: { allowed: boolean }) {
  return (
    <span style={{
      ...
      fontSize: 10, color: allowed ? 'var(--success)' : 'rgba(255,255,255,0.25)',
    }}>
      <span style={{ width: 5, height: 5, borderRadius: '50%', background: allowed ? 'var(--success)' : 'rgba(255,255,255,0.2)', flexShrink: 0 }} />
  ...
  return (
    <>
      <style>{`
        ...
        .am-box { background:var(--surface); border:0.5px solid rgba(251,146,60,0.2); border-radius:14px; width:100%; max-width:540px; max-height:92vh; overflow-y:auto; position:relative; box-shadow:0 24px 60px rgba(0,0,0,0.7); }
        ...
        .am-hdr { display:flex; align-items:center; justify-content:space-between; padding:16px 20px 12px; border-bottom:0.5px solid rgba(255,255,255,0.06); position:sticky; top:0; background:var(--surface); z-index:1; }
        .am-title { font-size:14px; font-weight:500; color:var(--text-strong); font-family:'IBM Plex Sans',sans-serif; }
        ...
        .am-input, .am-select { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.1); border-radius:7px; padding:9px 12px; font-size:13px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-strong); outline:none; width:100%; transition:border-color 0.2s, box-shadow 0.2s; }
        ...
        .am-select option { background:var(--surface); color:var(--text-strong); }
        ...
        .am-error { background:rgba(239,68,68,0.1); border:0.5px solid rgba(239,68,68,0.25); border-radius:7px; padding:8px 12px; font-size:12px; color:var(--danger-subtle); }
        ...
        .am-btn-save { background:linear-gradient(135deg,var(--accent-pressed),var(--accent),var(--accent-mid)); border:none; border-radius:7px; padding:8px 20px; font-size:13px; font-weight:500; font-family:'IBM Plex Sans',sans-serif; color:white; cursor:pointer; box-shadow:0 3px 12px rgba(234,88,12,0.35); transition:opacity 0.2s; }
              ...
              <div className="am-toggles">
                {([
                  ...
                  <label key={key} className="am-toggle-label"
                    style={{ color: form[key] ? 'var(--text-primary)' : 'rgba(255,255,255,0.4)' }}>
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'var(--surface)', border:'0.5px solid rgba(239,68,68,0.25)', borderRadius:14, width:'100%', maxWidth:420, padding:'24px 24px 20px', boxShadow:'0 24px 60px rgba(0,0,0,0.7)' }}>
        <div style={{ fontSize:14, fontWeight:500, color:'var(--text-strong)', marginBottom:10 }}>Delete account?</div>
        <div style={{ fontSize:13, color:'rgba(255,255,255,0.5)', marginBottom:20, lineHeight:1.5 }}>
          <strong style={{ color:'var(--text-strong)' }}>{account.accountNumber} — {account.name}</strong>{' '}
  ...
  return (
    <ERPShell breadcrumbs={['Home', 'Financial', 'Chart of Accounts']} title="Chart of Accounts">
      <style>{`
        ...
        .coa-stat-value { font-size:22px; font-weight:500; font-family:'IBM Plex Mono',monospace; color:var(--text-strong); }
        .coa-toolbar { display:flex; align-items:center; gap:10px; margin-bottom:14px; flex-wrap:wrap; }
        .coa-search { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-primary); outline:none; width:260px; transition:border-color 0.2s; }
        ...
        .coa-filter { background:rgba(255,255,255,0.04); border:0.5px solid rgba(255,255,255,0.09); border-radius:7px; padding:7px 12px; font-size:12px; font-family:'IBM Plex Sans',sans-serif; color:var(--text-primary); outline:none; }
        .coa-filter option { background:var(--surface); color:var(--text-strong); }
        .coa-btn-new { display:flex; align-items:center; gap:6px; margin-left:auto; background:linear-gradient(135deg,var(--accent-pressed),var(--accent),var(--accent-mid)); border:none; border-radius:7px; padding:7px 14px; font-size:12px; font-weight:500; font-family:'IBM Plex Sans',sans-serif; color:white; cursor:pointer; box-shadow:0 3px 12px rgba(234,88,12,0.3); transition:opacity 0.15s, transform 0.15s; flex-shrink:0; }
        ...
        .coa-num  { font-family:'IBM Plex Mono',monospace; font-size:13px; color:var(--accent-strong); font-weight:500; }
        .coa-name { color:var(--text-primary); font-weight:500; }
        ...
        .coa-btn-del { background:rgba(239,68,68,0.08); color:var(--danger); border-color:rgba(239,68,68,0.2); }
        ...
        .coa-spinner { width:18px; height:18px; border-radius:50%; border:2px solid rgba(251,146,60,0.2); border-top-color:var(--accent-strong); animation:coa-spin 0.7s linear infinite; flex-shrink:0; }
        ...
        .coa-error { background:rgba(239,68,68,0.08); border:0.5px solid rgba(239,68,68,0.2); border-radius:8px; padding:10px 14px; margin-bottom:14px; font-size:13px; color:var(--danger-subtle); }
      `}</style>
        ...
        {accounts.length > 0 && (
          <div className="coa-stats">
            ...
            >
              ...
              <span className="coa-stat-value" style={{ color: 'var(--accent-strong)' }}>{accounts.length}</span>
            </div>
          </div>
        )}
