...
const MODULE_REGISTRY: Record<string, {
  ...
  ar_invoice:          { label: 'AR Invoice',          description: 'JE when invoice is sent to customer',                   color: 'var(--accent-blue, #60a5fa)', group: 'Accounts Receivable', groupColor: 'var(--accent-blue, #60a5fa)' },
  ar_payment:          { label: 'AR Payment',          description: 'JE when payment is received on invoice',                color: 'var(--accent-blue, #60a5fa)', group: 'Accounts Receivable', groupColor: 'var(--accent-blue, #60a5fa)' },
  ar_reversal:         { label: 'AR Reversal',         description: 'JE when invoice is voided',                             color: 'var(--danger, #f87171)', group: 'Accounts Receivable', groupColor: 'var(--accent-blue, #60a5fa)' },
  ap_invoice:          { label: 'AP Invoice',          description: 'JE when AP invoice is posted (Inventory DR / AP CR)',   color: 'var(--accent-violet, #a78bfa)', group: 'Accounts Payable',    groupColor: 'var(--accent-violet, #a78bfa)' },
  ap_payment:          { label: 'AP Payment',          description: 'JE when supplier payment is applied (AP DR / Cash CR)', color: 'var(--accent-violet, #a78bfa)', group: 'Accounts Payable',    groupColor: 'var(--accent-violet, #a78bfa)' },
  ap_reversal:         { label: 'AP Reversal',         description: 'JE when AP invoice is voided',                         color: 'var(--danger, #f87171)', group: 'Accounts Payable',    groupColor: 'var(--accent-violet, #a78bfa)' },
  fg_delivery:         { label: 'FG Delivery',         description: 'JE when finished goods are delivered from MO',         color: 'var(--success, #4ade80)', group: 'Manufacturing',        groupColor: 'var(--success, #4ade80)' },
  production_variance: { label: 'Production Variance', description: 'JE when merma or surplus variance is posted',          color: 'var(--warning, #fbbf24)', group: 'Manufacturing',        groupColor: 'var(--success, #4ade80)' },
  po_receipt:          { label: 'PO Receipt',          description: 'JE when purchase order is received',                   color: 'var(--accent-strong, #fb923c)', group: 'Procurement',          groupColor: 'var(--accent-strong, #fb923c)' },
  mo_issue:            { label: 'MO Material Issue',   description: 'JE when materials are issued to production',           color: 'var(--accent-strong, #fb923c)', group: 'Procurement',          groupColor: 'var(--accent-strong, #fb923c)' },
};

const GROUP_ORDER = ['Accounts Receivable', 'Accounts Payable', 'Procurement', 'Manufacturing'];

const MODE_OPTIONS = [
  { value: 'full_auto',       label: 'Full Auto',       desc: 'Post JE immediately',        color: 'var(--success, #4ade80)' },
  { value: 'review_required', label: 'Review Required', desc: 'Draft + finance queue',      color: 'var(--warning, #fbbf24)' },
  { value: 'manual',          label: 'Manual',          desc: 'No auto-JE — finance posts', color: 'var(--danger, #f87171)' },
];
...
function modeStyle(mode: string): React.CSSProperties {
  ...
  return {
    color: opt?.color ?? 'var(--text-primary, #e2dfd8)',
    background: `${opt?.color ?? 'var(--text-primary, #e2dfd8)'}15`,
    border: `0.5px solid ${opt?.color ?? 'var(--text-primary, #e2dfd8)'}35`,
              ...
              >
                ...
                <div style={{ marginRight: 12 }}>
                  <span style={modeStyle(cfg.mode)}>
                    <span style={{ width: 5, height: 5, borderRadius: '50%', background: MODE_OPTIONS.find(o => o.value === cfg.mode)?.color ?? 'var(--text-primary, #e2dfd8)', flexShrink: 0 }} />
      ...
      <div className="auto-page">
        ...
        {!loading && (
          <div className="auto-stats">
            {[
              { label: 'Total Modules',    value: configs.length,  color: 'var(--text-strong, #f1ede8)' },
              { label: 'Enabled',          value: totalEnabled,    color: 'var(--success, #4ade80)' },
              { label: 'Full Auto',        value: totalAuto,       color: 'var(--success, #4ade80)' },
              { label: 'Review Required',  value: totalReview,     color: 'var(--warning, #fbbf24)' },
              { label: 'Manual',           value: configs.length - totalAuto - totalReview, color: 'var(--danger, #f87171)' },
            ].map(s => (
          ...
          {loading ? (
            ...
            <>
              ...
              {GROUP_ORDER.map(groupName => {
                ...
                const groupColor = meta?.groupColor ?? 'var(--accent-strong, #fb923c)';
                return (
                  <GroupNode
                    key={groupName}
                    groupName={groupName}
              ...
              {ungrouped.length > 0 && (
                <GroupNode
                  groupName="Other"
                  groupColor="var(--text-secondary, #6b7280)"
                  configs={ungrouped}
                  saving={saving}
                  onModeChange={handleModeChange}
                  onToggle={handleToggle}
