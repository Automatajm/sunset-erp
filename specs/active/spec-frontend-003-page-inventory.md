...
| 2026-06-20 | **P2–P4 adoption — remaining ConfirmModal guards.** Guarded every remaining unguarded destructive action with the shared `ConfirmModal`, one page per commit: sales-orders ship/deliver (create stock movements), sales/invoices send/void, production-plans cancel (+ replaced the emoji `alert()` MO-generation feedback with an inline notice banner), rfqs send/cancel (was native `confirm()`/`alert()`, also dropped the Send emoji), settings/roles delete (was `window.confirm`), manufacturing/bom whole-BOM delete (zero-guard) + routing-step delete, general-needs cancel (+ absorbed status `alert()` into an inline banner), accounting/budgets approve (irreversible), accounting/fiscal-periods close/reopen/lock/unlock, settings/tenants remove-user (was a swallowed direct delete), settings/users deactivate. Pattern: forward/restorative transitions stay direct; errors absorbed inline (no `alert`); PrintButton wiring untouched; no new deps. **cash-flow needs no guard** — its delete API has no UI button (matrix's "delete API w/o UI"). Full `tsc` clean; `pnpm build` green (2GB heap cap). This closes all 17 unguarded destructive actions from the 2026-06-06 audit. | 11 pages guarded; ConfirmModal adoption complete |
